import{_ as p,r as o,o as l,c as i,a as n,b as a,d as t,e}from"./app-CD_-ggd_.js";const c={},u=e(`<h1 id="深度学习-caffe分类模型训练、结果可视化、部署及量化笔记" tabindex="-1"><a class="header-anchor" href="#深度学习-caffe分类模型训练、结果可视化、部署及量化笔记"><span>[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记</span></a></h1><p>本文为本人caffe分类网络训练、结果可视化、部署及量化具体过程的心得笔记。caffe目前官方已经停止支持了，但是caffe是目前工业落地最常用的深度学习框架，用的人挺多。其实主要怕自己忘了，弄个备份，弄caffe很久了，怕不用东西都忘了，但是本文主要是讲述caffe下的分类网络。caffe默认已经配置好了，而且尽可能是linux系统，本文基于ubuntu18系统。如果有错误，希望积极指正。</p><h2 id="_1-训练" tabindex="-1"><a class="header-anchor" href="#_1-训练"><span>1 训练</span></a></h2><h3 id="_1-1-数据准备" tabindex="-1"><a class="header-anchor" href="#_1-1-数据准备"><span>1.1 数据准备</span></a></h3><p>首先在caffe/data路径建立example_data文件夹，在example_data里建立三个文件夹。 <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808153723769.png" alt="在这里插入图片描述" loading="lazy"> train文件为训练文件数据，val为验证文件数据，dataSet为最后生成caffe所用数据存放文件夹。这里准备五类数据，分别放在文件夹0-4。标号必须为0到4。val文件夹也是一样的。 <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808154146203.png" alt="在这里插入图片描述" loading="lazy"> 0到4文件夹存放各类图像，各类图像编号类似如下：<img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808164240294.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0x1b2hlbllK,size_16,color_FFFFFF,t_70" alt="在这里插入图片描述" loading="lazy"> 然后调用label.py创建数据集标签名，分别在train目录和val目录生成数据集标签文件，创建数据集的txt文件，如果是训练集命名为train.txt，如果是验证集命名为val.txt。label.py代码如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> os 
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>对于建立图像训练集可能会出现jpg文件损坏以及其他问题，一些小工具可以见：</p><blockquote><p>https://blog.csdn.net/LuohenYJ/article/details/86574451</p></blockquote><h3 id="_1-2-创建lmdb文件" tabindex="-1"><a class="header-anchor" href="#_1-2-创建lmdb文件"><span>1.2 创建lmdb文件</span></a></h3><p>将上一节得到的imageTrain文件夹和imageVal文件夹移动到dataSet目录。如下所示： 其中create_data.sh用于创建lmbdb文件，make_data_mean.sh用于创建均值文件。首先在caffe根目录下运行create_data.sh文件，命令如下： <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808170404250.png" alt="在这里插入图片描述" loading="lazy"> create_data.sh脚本具体内容如下，具体要改的参数都有标明，都是在caffe目录下使用相对路径名，绝对路径名容易出错。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">#!/usr/bin/env sh</span>
...
echo <span class="token string">&quot;Done.&quot;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>最后我们会得到均值数据文件，分别是BGR通道训练样本均值。记得把这三个值记一下以后要用到。 <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808171456251.png" alt="在这里插入图片描述" loading="lazy"> 所得到的文件lmdb和均值文件可以在example_data根目录下找到。训练模型用这三个文件就行了。 <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808171346570.png" alt="在这里插入图片描述" loading="lazy"></p><h3 id="_1-3-模型训练" tabindex="-1"><a class="header-anchor" href="#_1-3-模型训练"><span>1.3 模型训练</span></a></h3><p>训练时还需要solver文件来对参数进行更新， 同时还需要网络结构文件。在caffe/examples下建立example_data文件夹下进行训练。将前面的lmdb文件和均值文件复制到改文件夹下，如下图所示。橙色框选的文件是上步所获得的，红色框选的文件是网络结构参数文件和调参文件。backup是生成用于保存训练模型的文件夹，caffe.log是训练时生成的日志文件。 <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808192523910.png" alt="在这里插入图片描述" loading="lazy"> solver文件就用示例的配置alexnet文件。solver文件主要参数搜索就用，其中net值得是模型结构文件路径，训练习惯以caffe为根目录，snapshot_prefix为模型保存目录。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">## 网络结构地址</span>
...
layer <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>另外一个要改的就是结尾的num_output，将最后一个num_output改为分类个数，本文有5类所以num_output:5</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>layer <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="_1-4-训练部分参考文件" tabindex="-1"><a class="header-anchor" href="#_1-4-训练部分参考文件"><span>1.4 训练部分参考文件</span></a></h3><p>除了官方文件外，参考的内容有：</p>`,35),r={href:"https://www.cnblogs.com/wktwj/p/6715110.html",target:"_blank",rel:"noopener noreferrer"},d={href:"https://blog.csdn.net/cham_3/article/details/72141753",target:"_blank",rel:"noopener noreferrer"},k={href:"https://blog.csdn.net/hongbin_xu/article/details/79363134",target:"_blank",rel:"noopener noreferrer"},m={href:"https://blog.csdn.net/u014381600/article/details/54428599",target:"_blank",rel:"noopener noreferrer"},v={href:"https://zhuanlan.zhihu.com/p/48462756",target:"_blank",rel:"noopener noreferrer"},b={href:"https://blog.csdn.net/u010402786/article/details/70141261",target:"_blank",rel:"noopener noreferrer"},g={href:"https://www.cnblogs.com/louyihang-loves-baiyan/p/5038758.html",target:"_blank",rel:"noopener noreferrer"},_={href:"https://blog.csdn.net/Angela_qin/article/details/79428987",target:"_blank",rel:"noopener noreferrer"},h={href:"https://blog.csdn.net/nongfu_spring/article/details/51514040",target:"_blank",rel:"noopener noreferrer"},E={href:"https://blog.csdn.net/d5224/article/details/77100268",target:"_blank",rel:"noopener noreferrer"},f={href:"https://github.com/SnailTyan/caffe-model-zoo",target:"_blank",rel:"noopener noreferrer"},y={href:"https://github.com/mrgloom/kaggle-dogs-vs-cats-solution",target:"_blank",rel:"noopener noreferrer"},A=e(`<h2 id="_2-结果可视化" tabindex="-1"><a class="header-anchor" href="#_2-结果可视化"><span>2 结果可视化</span></a></h2><h3 id="_2-1-训练数据展示" tabindex="-1"><a class="header-anchor" href="#_2-1-训练数据展示"><span>2.1 训练数据展示</span></a></h3><p>在caffe的训练过程中，需要图形化训练数据结果。caffe中自带了工具显示结果。在examples/example_data下建立analyze文件，然后把训练过程生成的caffe.log移动到该文件夹。然后将caffe根目录tools/extra文件夹下的extract_seconds.py、parse_log.py、parse_log.sh和plot_training_log.py.example文件移动到该文件夹下。 <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190809094044741.png" alt="在这里插入图片描述" loading="lazy"> 在analyze当前目录下输入以下指令就可以可视化训练日志</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token punctuation">.</span><span class="token operator">/</span>plot_training_log<span class="token punctuation">.</span>py<span class="token punctuation">.</span>example <span class="token number">0</span>  save<span class="token punctuation">.</span>png caffe<span class="token punctuation">.</span>log
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>上面0表示可视化的类型，save.png表示可视化保存的图像名，caffe.log表示日志文件。 可视化类型具体参数如下。0、1、2等序号表示可视化类型，vs为横坐标参数，vs右边为纵坐标参数</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>    <span class="token number">0</span><span class="token punctuation">:</span> Test accuracy  vs<span class="token punctuation">.</span> Iters
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>这样将得到以下两个文件，分别表示训练数据和测试数据</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>caffe<span class="token punctuation">.</span>log<span class="token punctuation">.</span>train
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>对于单个分类曲线计算约登指数Youden Index（TPR-FPR或者TPR+TNR-1），取使得约登指数最大的阈值为最佳阈值。多分类ROC曲线绘制结果如下图所示，area为AUC值。另外多分类ROC曲线detectResult 用的是预测标签和真实标签对比绘制ROC曲线。 <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190809192312664.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0x1b2hlbllK,size_16,color_FFFFFF,t_70" alt="在这里插入图片描述" loading="lazy"></p><h3 id="_2-4-结果可视化部分参考文件" tabindex="-1"><a class="header-anchor" href="#_2-4-结果可视化部分参考文件"><span>2.4 结果可视化部分参考文件</span></a></h3>`,35),B={href:"https://blog.csdn.net/u013078356/article/details/51154847",target:"_blank",rel:"noopener noreferrer"},x={href:"https://blog.csdn.net/zziahgf/article/details/79215862",target:"_blank",rel:"noopener noreferrer"},w={href:"http://ethereon.github.io/netscope/#/editor",target:"_blank",rel:"noopener noreferrer"},F={href:"https://github.com/lutzroeder/Netron",target:"_blank",rel:"noopener noreferrer"},q={href:"https://www.jianshu.com/p/82903edb58dc",target:"_blank",rel:"noopener noreferrer"},C={href:"https://www.cnblogs.com/dlml/p/4403482.html",target:"_blank",rel:"noopener noreferrer"},R={href:"https://blog.csdn.net/xyz1584172808/article/details/81839230",target:"_blank",rel:"noopener noreferrer"},D={href:"https://blog.csdn.net/YE1215172385/article/details/79443552",target:"_blank",rel:"noopener noreferrer"},z=e(`<h2 id="_3-部署和量化" tabindex="-1"><a class="header-anchor" href="#_3-部署和量化"><span>3 部署和量化</span></a></h2><h3 id="_3-1-部署" tabindex="-1"><a class="header-anchor" href="#_3-1-部署"><span>3.1 部署</span></a></h3><p>部署有三种推荐方式，OpenCV DNN模块调用caffe模型/mini-caffe/ncnn。本文主要介绍DNN调用caffe模型和ncnn。 OpenCV调用caffe模型代码(dnn_test.cpp)如下：</p><div class="language-C++ line-numbers-mode" data-ext="C++" data-title="C++"><pre class="language-C++"><code>#include &quot;pch.h&quot;
#include &lt;iostream&gt;
#include &lt;fstream&gt;
#include &lt;sstream&gt;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>proto/model是caffe模型参数文件，mean为BGR均值，norm是caffe模型归一化参数，有些模型没有归一化就不要填，image是用于校准图像目录(jpg图像)。量化时需要校准，具体原理见:</p><blockquote><p>https://arleyzhang.github.io/articles/923e2c40/</p></blockquote><p>这样量化后获得输出文件model.table，在将caff模型转换为ncnn模型时，加入model.table，如下所示，就可以得到量化的模型。</p><blockquote><p>./caffe2ncnn deploy.prototxt model.caffemodel model_int8.param model_int8.bin 256 model.table</p></blockquote><p>上面256指的是quantizelevel量化级别，如果为0就不进行量化。如果某些层不想量化，打开model.table，输出那一层的量化参数。 <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190810110419375.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0x1b2hlbllK,size_16,color_FFFFFF,t_70" alt="在这里插入图片描述" loading="lazy"> 调用int量化后的模型，和原来ncnn调用不量化的模型一样，ncnn会自动调用量化模型。 如果不行量化模型或者模型量化后精度较低，可以在ncnn下对模型优化ncnnoptimize。也就是合并模型某些层，比如conv层和relu层，具体如下： https://github.com/Tencent/ncnn/wiki/model-optimize 调用命令很简单如下所示，其中65536表示2的16次方，即FP16，这表示模型优化还会将模型从float32转为float16。</p><div class="language-path line-numbers-mode" data-ext="path" data-title="path"><pre class="language-path"><code>ncnnoptimize model.param model.bin model-opt.param model-opt.bin 65536
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>对于squeezenet，就精度而言model和model-opt精度差不多，model_int8精度越降5%。就速度而言model-opt为model的95%，model_int8为model的90%。正常使用情况推荐： model&gt;model-opt&gt;model_int8</p><h3 id="_3-4-部署和量化部分参考文件" tabindex="-1"><a class="header-anchor" href="#_3-4-部署和量化部分参考文件"><span>3.4 部署和量化部分参考文件</span></a></h3>`,29),P={href:"https://blog.csdn.net/shanglianlm/article/details/80030569",target:"_blank",rel:"noopener noreferrer"},T={href:"https://www.jianshu.com/p/fdf9c3b70dd4",target:"_blank",rel:"noopener noreferrer"},L={href:"https://github.com/opencv/opencv/tree/master/samples/dnn",target:"_blank",rel:"noopener noreferrer"},G={href:"https://blog.csdn.net/weixin_45250844/article/details/94910897",target:"_blank",rel:"noopener noreferrer"},N={href:"https://blog.csdn.net/qq_36982160/article/details/79929869",target:"_blank",rel:"noopener noreferrer"},Z={href:"https://blog.csdn.net/u014644466/article/details/83278954",target:"_blank",rel:"noopener noreferrer"},O={href:"https://blog.csdn.net/qq_33431368/article/details/85029041",target:"_blank",rel:"noopener noreferrer"},I={href:"https://zhuanlan.zhihu.com/p/71881443",target:"_blank",rel:"noopener noreferrer"},V=n("h2",{id:"_4-总结",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#_4-总结"},[n("span",null,"4 总结")])],-1),M=n("p",null,"本文只写了caffe分类模型的相关笔记，目标检测就没写了。因为用darknet的yolo比较多。有空在写caffe目标检测的笔记。",-1);function H(j,S){const s=o("ExternalLinkIcon");return l(),i("div",null,[u,n("ul",null,[n("li",null,[n("a",r,[a("训练自己的数据集"),t(s)])]),n("li",null,[n("a",d,[a("caffe入门"),t(s)])]),n("li",null,[n("a",k,[a("caffe官方教程"),t(s)])]),n("li",null,[n("a",m,[a("solver参数设置"),t(s)])]),n("li",null,[n("a",v,[a("solver参数说明"),t(s)])]),n("li",null,[n("a",b,[a("模型微调1"),t(s)])]),n("li",null,[n("a",g,[a("模型微调2"),t(s)])]),n("li",null,[n("a",_,[a("模型微调3"),t(s)])]),n("li",null,[n("a",h,[a("模型微调4"),t(s)])]),n("li",null,[n("a",E,[a("经典网络总结"),t(s)])]),n("li",null,[n("a",f,[a("经典网络模型"),t(s)])]),n("li",null,[n("a",y,[a("caffe猫狗大战训练"),t(s)])])]),A,n("ul",null,[n("li",null,[n("a",B,[a("caffe训练日志可视化1"),t(s)])]),n("li",null,[n("a",x,[a("caffe训练日志可视化2"),t(s)])]),n("li",null,[n("a",w,[a("Netscope"),t(s)])]),n("li",null,[n("a",F,[a("Netron"),t(s)])]),n("li",null,[n("a",q,[a("ROC理论1"),t(s)])]),n("li",null,[n("a",C,[a("ROC理论2"),t(s)])]),n("li",null,[n("a",R,[a("ROC曲线绘制1"),t(s)])]),n("li",null,[n("a",D,[a("ROC曲线绘制2"),t(s)])])]),z,n("ul",null,[n("li",null,[n("a",P,[a("OpenCV调用Caffe1"),t(s)])]),n("li",null,[n("a",T,[a("OpenCV调用Caffe2"),t(s)])]),n("li",null,[n("a",L,[a("OpenCV深度学习调用实例"),t(s)])]),n("li",null,[n("a",G,[a("ncnn入门1"),t(s)])]),n("li",null,[n("a",N,[a("ncnn入门2"),t(s)])]),n("li",null,[n("a",Z,[a("caffe量化1"),t(s)])]),n("li",null,[n("a",O,[a("caffe量化2"),t(s)])]),n("li",null,[n("a",I,[a("ncnn量化详解"),t(s)])])]),V,M])}const K=p(c,[["render",H],["__file","2019-08-10-_深度学习_ caffe分类模型训练、结果可视化、部署及量化笔记.html.vue"]]),$=JSON.parse('{"path":"/blog/%E6%B7%B1%E5%BA%A6%E5%AD%A6%E4%B9%A0/2019-08-10-_%E6%B7%B1%E5%BA%A6%E5%AD%A6%E4%B9%A0_%20caffe%E5%88%86%E7%B1%BB%E6%A8%A1%E5%9E%8B%E8%AE%AD%E7%BB%83%E3%80%81%E7%BB%93%E6%9E%9C%E5%8F%AF%E8%A7%86%E5%8C%96%E3%80%81%E9%83%A8%E7%BD%B2%E5%8F%8A%E9%87%8F%E5%8C%96%E7%AC%94%E8%AE%B0.html","title":"[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记","lang":"zh-CN","frontmatter":{"date":"2019-08-10T11:23:59.000Z","category":["深度学习"],"tag":["深度学习","常用工具"],"description":"[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记 本文为本人caffe分类网络训练、结果可视化、部署及量化具体过程的心得笔记。caffe目前官方已经停止支持了，但是caffe是目前工业落地最常用的深度学习框架，用的人挺多。其实主要怕自己忘了，弄个备份，弄caffe很久了，怕不用东西都忘了，但是本文主要是讲述caffe下的分类网络。caf...","head":[["meta",{"property":"og:url","content":"https://luohenyueji.github.io/blog/%E6%B7%B1%E5%BA%A6%E5%AD%A6%E4%B9%A0/2019-08-10-_%E6%B7%B1%E5%BA%A6%E5%AD%A6%E4%B9%A0_%20caffe%E5%88%86%E7%B1%BB%E6%A8%A1%E5%9E%8B%E8%AE%AD%E7%BB%83%E3%80%81%E7%BB%93%E6%9E%9C%E5%8F%AF%E8%A7%86%E5%8C%96%E3%80%81%E9%83%A8%E7%BD%B2%E5%8F%8A%E9%87%8F%E5%8C%96%E7%AC%94%E8%AE%B0.html"}],["meta",{"property":"og:site_name","content":"落痕月极的博客"}],["meta",{"property":"og:title","content":"[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记"}],["meta",{"property":"og:description","content":"[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记 本文为本人caffe分类网络训练、结果可视化、部署及量化具体过程的心得笔记。caffe目前官方已经停止支持了，但是caffe是目前工业落地最常用的深度学习框架，用的人挺多。其实主要怕自己忘了，弄个备份，弄caffe很久了，怕不用东西都忘了，但是本文主要是讲述caffe下的分类网络。caf..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog
slug":"_1-2-创建lmdb文件","link":"#_1-2-创建lmdb文件","children":[]},{"level":3,"title":"1.3 模型训练","slug":"_1-3-模型训练","link":"#_1-3-模型训练","children":[]},{"level":3,"title":"1.4 训练部分参考文件","slug":"_1-4-训练部分参考文件","link":"#_1-4-训练部分参考文件","children":[]}]},{"level":2,"title":"2 结果可视化","slug":"_2-结果可视化","link":"#_2-结果可视化","children":[{"level":3,"title":"2.1 训练数据展示","slug":"_2-1-训练数据展示","link":"#_2-1-训练数据展示","children":[]},{"level":3,"title":"2.2 网络模型可视化","slug":"_2-2-网络模型可视化","link":"#_2-2-网络模型可视化","children":[]},{"level":3,"title":"2.3 ROC曲线绘制","slug":"_2-3-roc曲线绘制","link":"#_2-3-roc曲线绘制","children":[]},{"level":3,"title":"2.4 结果可视化部分参考文件","slug":"_2-4-结果可视化部分参考文件","link":"#_2-4-结果可视化部分参考文件","children":[]}]},{"level":2,"title":"3 部署和量化","slug":"_3-部署和量化","link":"#_3-部署和量化","children":[{"level":3,"title":"3.1 部署","slug":"_3-1-部署","link":"#_3-1-部署","children":[]},{"level":3,"title":"3.2 量化","slug":"_3-2-量化","link":"#_3-2-量化","children":[]},{"level":3,"title":"3.4 部署和量化部分参考文件","slug":"_3-4-部署和量化部分参考文件","link":"#_3-4-部署和量化部分参考文件","children":[]}]},{"level":2,"title":"4 总结","slug":"_4-总结","link":"#_4-总结","children":[]}],"git":{},"readingTime":{"minutes":28.93,"words":8680},"filePathRelative":"blog/深度学习/2019-08-10-[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记.md","localizedDate":"2019年8月10日","excerpt":"\\n<p>本文为本人caffe分类网络训练、结果可视化、部署及量化具体过程的心得笔记。caffe目前官方已经停止支持了，但是caffe是目前工业落地最常用的深度学习框架，用的人挺多。其实主要怕自己忘了，弄个备份，弄caffe很久了，怕不用东西都忘了，但是本文主要是讲述caffe下的分类网络。caffe默认已经配置好了，而且尽可能是linux系统，本文基于ubuntu18系统。如果有错误，希望积极指正。</p>\\n<h2>1 训练</h2>\\n<h3>1.1 数据准备</h3>\\n<p>首先在caffe/data路径建立example_data文件夹，在example_data里建立三个文件夹。\\n<img src=\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808153723769.png\\" alt=\\"在这里插入图片描述\\" loading=\\"lazy\\">\\ntrain文件为训练文件数据，val为验证文件数据，dataSet为最后生成caffe所用数据存放文件夹。这里准备五类数据，分别放在文件夹0-4。标号必须为0到4。val文件夹也是一样的。\\n<img src=\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808154146203.png\\" alt=\\"在这里插入图片描述\\" loading=\\"lazy\\">\\n0到4文件夹存放各类图像，各类图像编号类似如下：<img src=\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[深度学习] caffe分类模型训练、结果可视化、部署及量化笔记/20190808164240294.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0x1b2hlbllK,size_16,color_FFFFFF,t_70\\" alt=\\"在这里插入图片描述\\" loading=\\"lazy\\">\\n然后调用label.py创建数据集标签名，分别在train目录和val目录生成数据集标签文件，创建数据集的txt文件，如果是训练集命名为train.txt，如果是验证集命名为val.txt。label.py代码如下：</p>","autoDesc":true}');export{K as comp,$ as data};
