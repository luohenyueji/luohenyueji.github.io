import{_ as o,r,o as i,c as l,a as n,b as e,d as a,e as s}from"./app-CD_-ggd_.js";const p={},c=s('<h1 id="opencv实战-8-深度学习目标检测网络yolov3的训练" tabindex="-1"><a class="header-anchor" href="#opencv实战-8-深度学习目标检测网络yolov3的训练"><span>[OpenCV实战]8 深度学习目标检测网络YOLOv3的训练</span></a></h1><p>YOLOv3是计算机视觉中最受欢迎的实时目标检测器之一。在这个循序渐进的教程中，我们从如何使用YOLOv3训练1类物体探测器的简单案例开始。本教程是以初学者为中心编写的。建立自己的雪人检测器。</p><p>在这篇文章中，我们将分享训练过程，有助于训练的脚本以及一些公开的雪人图像和视频。您可以使用相同的过程来训练具有多个对象的对象检测器。</p><p><strong>特别要注意的是，训练yolov3最好在linux下进行，同时所有涉及到输入路径的都最好使用绝对路径。</strong></p><h2 id="_1-数据集" tabindex="-1"><a class="header-anchor" href="#_1-数据集"><span><strong>1</strong> <strong>数据集</strong></span></a></h2><p>与任何深度学习任务一样，第一个最重要的任务是准备数据集。我们将使用Google的OpenImagesV4数据集中的雪人图像，这些图像可在线公开获取。它是一个非常大的数据集，包含大约600种不同的对象类。数据集还包含这些对象的边界框注释。总的来说，数据集超过500GB，但我们只会下载带有“雪人”对象的图像。</p><p>我们不拥有这些图像的版权，因此我们遵循共享源图像而不是图像文件本身的标准做法。OpenImages具有每个图像的原始URL和许可证信息。任何使用此数据（学术，非商业或商业）的风险均由您自行承担。</p><p>OpenImagesV4详细介绍：</p>',8),d={href:"http://storage.googleapis.com/openimages/web/challenge.html",target:"_blank",rel:"noopener noreferrer"},g=n("p",null,"然后你可以进入这个网站看看各种所有分类图像什么样子",-1),h={href:"https://storage.googleapis.com/openimages/web/challenge_visualizer/index.html?set=detection&c=%2Fm%2F04zwwv",target:"_blank",rel:"noopener noreferrer"},u=s(`<h3 id="_1-1-下载openimages-雪人数据-约1-5-小时" tabindex="-1"><a class="header-anchor" href="#_1-1-下载openimages-雪人数据-约1-5-小时"><span><strong>1.1</strong> <strong>下载openImages</strong> <strong>雪人数据[</strong> <strong>约1.5</strong> <strong>小时]</strong></span></a></h3><p>首先，我们需要安装awscli。必须用python3</p><pre><code>linux: sudo pip3 install awscli
...
</code></pre><p>改动的意思是，原始训练过程在每100次训练后保存网络权重，直到前1000次。然后接下来仅在每10000次训练后才保存。在我们的例子中，由于我们只训练一个类，我们希望我们的训练收敛得更快。因此，为了密切监视进度，我们在每200次迭代后保存，直到达到2000，然后接下来在每1000次迭代后保存。完成上述更改后，再次使用make命令重新编译darknet。</p><h3 id="_2-3-数据注释" tabindex="-1"><a class="header-anchor" href="#_2-3-数据注释"><span><strong>2.3 数据注释</strong></span></a></h3><p>我们在labels文件夹中共享带有注释的标签文件。标签文件中的每个行条目表示图像中的单个边界框，并包含有关该框的以下信息：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>&lt;object-class-id&gt; &lt;center-x&gt; &lt;center-y&gt; &lt;width&gt; &lt;height&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>第一个字段object-class-id是表示类别的整数。范围从0到（类别总数-1）。在我们目前的情况下，由于我们只有一类雪人，因此总是设置object-class-id为0。</p><p>center-x和center-y分别是边界框中心的x和y坐标，除以图像宽度和高度后的结果。</p><p>width和height分别是边界框的宽度和高度，除以图像宽度和高度后的结果。</p><p>让我们考虑一个带有以下符号的示例：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>x-边界框中心的x坐标（以像素为单位）
...
H-整个图像的高度（以像素为单位）
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后我们计算标签文件中的注释值，如下所示：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>center-x= x /W.
