import{_ as e,c as n,a,o}from"./app-BmRggoXf.js";const r={};function l(i,t){return o(),n("div",null,t[0]||(t[0]=[a(`<h1 id="opencv实战-27-在opencv下使用foreach进行并行像素访问" tabindex="-1"><a class="header-anchor" href="#opencv实战-27-在opencv下使用foreach进行并行像素访问"><span>[OpenCV实战]27 在OpenCV下使用forEach进行并行像素访问</span></a></h1><p>C++11扩展了for语句的语法。用这个新写法forEach，forEach可以遍历C类型的数组、初始化列表以及任何重载了非成员的begin()和end()函数的类型。OpenCV的Mat数据结构中有用到ForEach的写法。在本教程中，我们将比较Mat类的forEach方法与OpenCV中访问和转换像素值的其他方法的性能。我们将展示forEach如何比使用at方法或甚至有效地使用指针算法更快。因此本文只有C++ forEach用法介绍。Python下实现容易多，搜索即可。</p><p>OpenCV中有隐藏的功能，有时候并不是很有名。其中一个隐藏的功能是Mat类的forEach方法，它利用机器上的所有核心在每个像素上处理任何功能。</p><p>我们先来定义一个函数complexThreshold。它接收RGB像素值并对其应用复杂的阈值分割。代码如下：</p><pre><code>// Define a pixel 
