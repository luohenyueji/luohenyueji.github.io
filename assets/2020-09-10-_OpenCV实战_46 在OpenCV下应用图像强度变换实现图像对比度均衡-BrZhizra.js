...
<span class="line"><span>cv2.intensity_transform.contrastStretching(g_image, g_contrastStretch, g_r1, g_s1, g_r2, g_s2)</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>不同的方法所需要设定的参数不同，具体如下：</p><ul><li>autoscaling：对输入图像进行自适应缩放以增强对比度，仅需要输入待增强的图像。</li><li>gamma变换：对输入图像进行伽马校正以增强对比度，需要输入待增强图像和参数gamma。</li><li>对数变换：对输入图像进行对数转换以增强对比度，仅需要输入待增强的图像。</li><li>对比度拉伸：对输入图像应用线性对比度拉伸以增强对比度，需要输入待增强图像和参数r1，s1，r2，s2，(r1,s1)和(r2,s2)为转换函数第一个点和第二个点的坐标。</li></ul><p><strong>此外为了比较不同图像强度增强方法的效果，加入了图像对比度计算方法</strong></p><p>图像对比度计算方法为RMS Contrast，来自于<a href="https://stackoverflow.com/questions/58821130/how-to-calculate-the-contrast-of-an-image" target="_blank" rel="noopener noreferrer">How to calculate the contrast of an image?</a></p><p>方法原理很简单,就是将图像变为灰度图，然后计算图像方差。</p><h3 id="_2-2-完整代码" tabindex="-1"><a class="header-anchor" href="#_2-2-完整代码"><span>2.2 完整代码</span></a></h3><p>代码功能很简单，就是获得输入图像，然后对输入图像应用不同的图像增强算法。对于可调参数的，创建滑动条以调整方法的输入参数。但是要注意的是，输入图像必须为三通道RGB图像。C++和Python代码如下：</p><p><strong>C++</strong></p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code class="language-"><span class="line"><span>#include &lt;opencv2/opencv.hpp&gt;</span></span>
