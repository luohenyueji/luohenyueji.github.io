import{_ as o}from"./plugin-vue_export-helper-x3n3nnut.js";import{r as i,o as s,c,a as t,b as n,d as r,e as a}from"./app-MsA2k2kn.js";const d={},l=a('<h1 id="opencv实战-6-基于特征点匹配的视频稳像" tabindex="-1"><a class="header-anchor" href="#opencv实战-6-基于特征点匹配的视频稳像" aria-hidden="true">#</a> [OpenCV实战]6 基于特征点匹配的视频稳像</h1><p>在这篇文章中，我们将学习如何使用OpenCV库中称为特征点匹配的技术以实现简单视频稳定稳像。我们将讨论该算法并共享代码，以便在OpenCV中使用此方法设计一个简单的稳定器，最好OpenCV3.4.3以上实现代码。什么是视频稳定，视频稳定是指用于减少相机运动对最终视频影响的一系列方法，理解成消除视频抖动就行了。见下图通常用拍摄会出现轻微的抖动，比如手机拍摄视频，后期需要对其进行视频稳像操作。</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]6 基于特征点匹配的视频稳像/1.gif" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>视频稳定的需求涉及许多领域。它在消费者和专业摄像中极为重要。因此，存在许多不同的机械，光学和算法解决方案。即使在静态图像拍摄中，稳定也可以帮助拍摄时间长的照片。在内窥镜检查和结肠镜检查等医疗诊断应用中，需要稳定视频以确定问题的确切位置和宽度。类似地，在军事应用中，飞行器在侦察飞行中捕获的视频需要稳定以进行定位，导航，目标跟踪等。这同样适用于机器人应用。</p><h2 id="_1-介绍" tabindex="-1"><a class="header-anchor" href="#_1-介绍" aria-hidden="true">#</a> <strong>1</strong> <strong>介绍</strong></h2><h3 id="_1-1-视频稳定的方法" tabindex="-1"><a class="header-anchor" href="#_1-1-视频稳定的方法" aria-hidden="true">#</a> <strong>1.1</strong> <strong>视频稳定的方法</strong></h3><p>视频稳定方法包括机械，光学和数字稳定方法。具体如下：</p><p>机械视频稳定：机械图像稳定系统使用陀螺仪和加速度计等特殊传感器检测到运动来移动图像传感器以补偿相机的运动。</p><p>光学视频稳定：在这种方法中，不是移动整个相机，而是通过移动镜头的部分来实现稳定。该方法采用可移动透镜组件，当透过相机的透镜系统时，可移动透镜组件可变地调节光的路径长度。</p><p>数字视频稳定：此方法不需要特殊的传感器来估算相机运动。本文就是用的这种方法。主要有三个步骤， 1）运动估计，2）运动平滑，3）图像合成。在第一阶段中导出两个连续帧之间的变换参数。第二级滤除不需要的运动，第三个阶段重建稳定的视频。</p><p>我们将在本文中学习快速而强大的实数字视频稳定算法。它基于二维运动模型，我们应用包含平移，旋转和缩放的欧几里德（又称相似性）变换。</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]6 基于特征点匹配的视频稳像/2.jpg" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>如上图所示，在欧几里德运动模型中，图像中的正方形可以转换为具有不同大小，形状位置的任何其他四方形。它比仿射和单应变换更具限制性，但足以用于运动稳定，因为视频的连续帧之间的相机移动通常很小。</p><h3 id="_1-2-使用点特征匹配的视频稳定" tabindex="-1"><a class="header-anchor" href="#_1-2-使用点特征匹配的视频稳定" aria-hidden="true">#</a> <strong>1.2</strong> <strong>使用点特征匹配的视频稳定</strong></h3><p>该方法涉及跟踪两个连续帧之间的一些特征点。跟踪的特征允许我们估计帧之间的运动并对其进行补偿。下面的流程图显示了算法基本步骤。</p><p>1 获取多帧视频图像，获取图像角点(特征点)；</p><p>2 光流法跟踪角点；根据前后两张图像角点变化得到表示运动的仿射变化矩阵。</p><p>3 根据仿射变化矩阵计算运动轨迹，并且平滑运动轨迹。</p><p>4 根据平滑后的运动轨迹，得到平滑运动后的仿射变化矩阵。</p><p>5 根据平滑运动后的仿射变化矩阵得到稳定后的图像。</p><figure><img src="https://imgconvert.csdnimg.cn/aHR0cHM6Ly93d3cubGVhcm5vcGVuY3YuY29tL3dwLWNvbnRlbnQvdXBsb2Fkcy8yMDE5LzAxL0FFQU0tMy5wbmc?x-oss-process=image/format,png" alt="https://www.learnopencv.com/wp-content/uploads/2019/01/AEAM-3.png" tabindex="0" loading="lazy"><figcaption>https://www.learnopencv.com/wp-content/uploads/2019/01/AEAM-3.png</figcaption></figure><h2 id="_2-算法" tabindex="-1"><a class="header-anchor" href="#_2-算法" aria-hidden="true">#</a> 2 算法</h2><h3 id="_2-1-帧间运动信息获取" tabindex="-1"><a class="header-anchor" href="#_2-1-帧间运动信息获取" aria-hidden="true">#</a> 2.1 帧间运动信息获取</h3><p>算法中最关键的部分是确定各帧的运动方向。我们将迭代所有帧，并找到当前帧和前一帧之间的运动。没有必要知道每个像素的运动。欧几里德运动模型要求我们知道两帧中2个点的运动信息。然而，在实践中最好找到50-100点的运动信息，然后使用它们来稳健地估计运动模型</p><h4 id="_2-1-1-合适的特征点获取" tabindex="-1"><a class="header-anchor" href="#_2-1-1-合适的特征点获取" aria-hidden="true">#</a> 2.1.1 合适的特征点获取</h4><p>现在的问题是我们应该选择哪些特征点进行跟踪。请记住，跟踪算法会使用一个以该点为圆心的圆(小孔)来近似来模拟点的运动。这种跟踪算法受到圆直径的影响。因此，平滑区域对于跟踪是不利的，并且具有许多角点的纹理区域是好的。幸运的是，OpenCV具有快速检测函数特征点的跟踪，即函数goodFeaturesToTrack。角点个人理解是指图像中亮度变化剧烈的点或图像边缘上变化交大的点。</p><h4 id="_2-1-2-lucas-kanade光流法" tabindex="-1"><a class="header-anchor" href="#_2-1-2-lucas-kanade光流法" aria-hidden="true">#</a> 2.1.2 Lucas-Kanade光流法</h4><p>一旦我们在前一帧中找到了好的特征(角点)，我们就可以使用名为Lucas-Kanade Optical Flow的算法在下一帧中跟踪它们，该算法以算法的发明者命名。算法详情可以见</p>',28),u={href:"https://blog.csdn.net/linmingan/article/details/79296963",target:"_blank",rel:"noopener noreferrer"},p=t("p",null,"OpenCV中的函数calcOpticalFlowPyrLK实现算法。在calcOpticalFlowPyrLK中，LK代表Lucas- Kanade，而Pyr代表金字塔。计算机视觉中的图像金字塔用于处理不同比例（分辨率）的图像。但是由于各种原因，calcOpticalFlowPyrLK可能无法计算所有点的运动。例如，当前帧中的特征点可能被下一帧中的另一个对象遮挡。幸运的是calcOpticalFlowPyrLK中的状态标志可用于过滤掉这些值。",-1),m=t("h4",{id:"_2-1-3-运动估计",tabindex:"-1"},[t("a",{class:"header-anchor",href:"#_2-1-3-运动估计","aria-hidden":"true"},"#"),n(" 2.1.3 运动估计")],-1),f=t("p",null,"回顾一下，在步骤2.1.1中，我们发现在前一帧中要跟踪的特征点。在步骤2.1.2中，我们使用光流来跟踪特征点。换句话说，我们在当前帧中找到了特征点的位置，并且我们已经知道了前一帧中特征点的位置。因此，我们可以使用这两组特征点来找到将前一帧映射到当前帧的（欧几里德）变换。OpenCV使用函数estimateRigidTransform完成的。仿射变化详细见",-1),_={href:"https://blog.csdn.net/dongfang1984/article/details/52959308",target:"_blank",rel:"noopener noreferrer"},g=a(`<p>一旦我们获取运动信息，我们就可以将它分解为x和y值以及平移和旋转（角度）值。我们将这些值存储在一个数组中，以便我们可以顺利更改它们。</p><p>下面的代码将介绍步骤2.1.1至2.1.3。请务必阅读要遵循的代码中的注释。</p><p>在C++实现中，我们首先定义一些类来帮助我们存储估计的运动矢量。下面C++代码的TransformParam类存储运动信息（dx为x轴运动信息，dy为y轴运动信息，da为角度信息），并提供方法getTransform将此运动转换为变换矩阵。</p><pre><code>/**
 * @brief 运动信息结构体
 *
 */
...
</code></pre><h3 id="_2-2-计算帧之间的总体运动" tabindex="-1"><a class="header-anchor" href="#_2-2-计算帧之间的总体运动" aria-hidden="true">#</a> <strong>2.2</strong> <strong>计算帧之间的总体运动</strong></h3><p>在上一步中，我们获取了帧之间的运动情况并将它们存储在一个数组中。我们现在需要通过累积分析帧间运动情况来找到运动轨迹。</p><h4 id="_2-2-1-计算运动轨迹" tabindex="-1"><a class="header-anchor" href="#_2-2-1-计算运动轨迹" aria-hidden="true">#</a> 2.2.1 计算运动轨迹</h4><p>在此步骤中，我们将累加帧之间的运动以计算轨迹。我们的最终目标是平滑这一轨迹。</p><p>在Python中，使用numpy中的cumsum（累积和）很容易实现。</p><p>在C ++中，我们定义了一个名为Trajectory的类来存储运动参数每次的累积和。</p><pre><code>/**
 * @brief 轨迹结构体
 *
 */
struct Trajectory
...
</code></pre><h4 id="_2-2-3-平滑变化计算" tabindex="-1"><a class="header-anchor" href="#_2-2-3-平滑变化计算" aria-hidden="true">#</a> 2.2.3 平滑变化计算</h4><p>到目前为止，我们已经获得了平滑的轨迹。在此步骤中，我们将使用平滑轨迹来获得平滑变换，这些变换可应用于视频帧以使其稳定。这是通过找到平滑轨迹和原始轨迹之间的差异并将该差异添加原始变换矩阵来完成的。C++代码：</p><pre><code>	//平滑后的运动信息结构体
...
</code></pre><h3 id="_2-3-将平滑后的变化矩阵应用于帧" tabindex="-1"><a class="header-anchor" href="#_2-3-将平滑后的变化矩阵应用于帧" aria-hidden="true">#</a> <strong>2.3</strong> <strong>将平滑后的变化矩阵应用于帧</strong></h3><p>我们差不多完成了。我们现在需要做的就是遍历帧并应用我们刚刚计算的变换。</p><p>如果我们将运动指定为 <em>x</em> , <em>y</em> , <em>θ</em> <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]6 基于特征点匹配的视频稳像/6.png" alt="" loading="lazy"> ，则相应的变换矩阵由下式给出：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]6 基于特征点匹配的视频稳像/7.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>当我们稳定视频时，我们可能会看到一些黑色边界。这是预期的，因为要稳定视频，视频帧原图像可能不得不缩小（不是图的尺寸缩小，图像尺寸不变。有两种情况，一种实际先缩小图像，然后从中截取原图大小的区域，缺少图像区域用黑色填充，起到图像增大作用；另外将原图扩大，然后截取原图尺寸相等大小区域，起到图像缩小作用）。我们可以通过以其视频中点为中心缩放图像（例如4％）来缓解该问题。下面的函数fixBorder显示了实现。我们使用getRotationMatrix2D，因为它可以在不移动图像中心的情况下缩放和旋转图像。我们需要做的就是调用此函数，旋转0和缩放1.04(将原图扩大为1.04倍，然后截取原图尺寸相等大小区域)。C++代码如下：</p><pre><code>/**
 * @brief 
 * 
...
</code></pre><h2 id="_3-结果和代码" tabindex="-1"><a class="header-anchor" href="#_3-结果和代码" aria-hidden="true">#</a> 3 结果和代码</h2><h3 id="_3-1-总结" tabindex="-1"><a class="header-anchor" href="#_3-1-总结" aria-hidden="true">#</a> 3.1 总结</h3><p>优点：</p><ul><li>该方法对低频运动（较慢的振动）提供了良好的稳定性。</li><li>该方法具有低内存消耗，因此非常适用于嵌入式设备（如Raspberry Pi）。</li><li>此方法可以很好地防止视频中的缩放（缩放）抖动。</li></ul><p>缺点：</p><ul><li>该方法对高频扰动的影响很小。</li><li>速度过慢。</li><li>如果运动模糊，则功能跟踪将失败，结果将不是最佳。</li><li>滚动快门失真也不适合这种方法。</li></ul><h3 id="_3-2-代码" tabindex="-1"><a class="header-anchor" href="#_3-2-代码" aria-hidden="true">#</a> 3.2 代码</h3><p>代码地址：</p>`,42),h={href:"https://download.csdn.net/download/luohenyj/11007133",target:"_blank",rel:"noopener noreferrer"},y={href:"https://github.com/luohenyueji/OpenCV-Practical-Exercise",target:"_blank",rel:"noopener noreferrer"},v=a(`<p>如果没有积分（系统自动设定资源分数）看看参考链接。我搬运过来的，大修改没有。</p><p>代码提供了C++和Python版本，代码都有详细的注释。</p><p><strong>但是需要注意的是提供的示例detect.mp4在1300帧后有丢帧现象，有些OpenCV版本会出问题，所以设定n_frames=1300只读取前1300帧，运行其他视频需要注意n_frames 的手动设定。</strong></p><p>C++：</p><pre><code>// video_stabilization.cpp : 此文件包含 &quot;main&quot; 函数。程序执行将在此处开始并结束。
//
...
</code></pre><h2 id="_4-参考" tabindex="-1"><a class="header-anchor" href="#_4-参考" aria-hidden="true">#</a> 4 参考</h2>`,8),b={href:"https://www.learnopencv.com/video-stabilization-using-point-feature-matching-in-opencv/",target:"_blank",rel:"noopener noreferrer"};function x(T,j){const e=i("ExternalLinkIcon");return s(),c("div",null,[l,t("p",null,[t("a",u,[n(" https://blog.csdn.net/linmingan/article/details/79296963"),r(e)])]),p,m,f,t("p",null,[t("a",_,[n(" https://blog.csdn.net/dongfang1984/article/details/52959308 "),r(e)])]),g,t("p",null,[t("a",h,[n(" https://download.csdn.net/download/luohenyj/11007133 "),r(e)])]),t("p",null,[t("a",y,[n(" https://github.com/luohenyueji/OpenCV-Practical-Exercise "),r(e)])]),v,t("ul",null,[t("li",null,[t("a",b,[n(" https://www.learnopencv.com/video-stabilization-using-point-feature-matching-in-opencv/ "),r(e)])])])])}const P=o(d,[["render",x],["__file","2019-03-08-_OpenCV实战_6 基于特征点匹配的视频稳像.html.vue"]]);export{P as default};
