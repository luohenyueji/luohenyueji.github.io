...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_1-python模块概述" tabindex="-1"><a class="header-anchor" href="#_1-python模块概述"><span>1 Python模块概述</span></a></h2><p>实际上，在Python中定义模块的方式有三种：</p><ol><li>可以使用Python本身编写模块。</li><li>可以使用C语言编写模块，并在运行时动态加载该模块，例如re（正则表达式）模块。</li><li>内置模块本质上包含在解释器中，就像itertools模块一样。</li></ol><p>在所有三种情况下，都以相同的方式访问模块的内容：使用import语句。在这里，重点将主要放在用Python编写的模块上。用Python编写的模块的妙处在于它们的构建极其简单。您需要做的就是创建一个包含合法Python代码的文件，然后为该文件命名并带有.py扩展名。</p><p>例如，假设有一个mod.py文件包含以下内容：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>s = &quot;Hello world!&quot;
a = [100, 200, 300]
...
<span class="token keyword">import</span> mod
 ...
 &#39;/home/aistudio/.ipython&#39;,
 &#39;./data&#39;]
</code></pre><p>导入模块后，您可以使用模块的__file__属性确定找到模块的位置：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> mod
...
</code></pre><h3 id="_2-2-import声明" tabindex="-1"><a class="header-anchor" href="#_2-2-import声明"><span>2.2 import声明</span></a></h3><p>import语句可将模块内容提供给调用方。该import语句采用许多不同的形式，如下所示。</p><p><strong>import &lt;module_name&gt;</strong></p><p>最简单的形式是上面已经显示的形式。请注意，这不会使调用者可以直接访问模块内容。每个模块都有其自己的专用符号表，该符号表用作模块中定义的所有对象的全局符号表。因此，模块已经创建了一个单独的名称空间。该语句import &lt;module_name&gt;仅放置&lt;module_name&gt;在调用者的符号表中。模块中定义的对象保留在模块的专用符号表中。该段话的意思就是你调用的模块变量和本地变量不冲突。</p><p>从调用方来看，只有通过点表示法以&lt;module_name&gt;作为前缀，才能访问模块中的对象，如下所示。在下面的import语句之后，mod被放置到本地符号表中。因此，mod在调用方的本地上下文中有意义:</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> mod
mod
...
</code></pre><p>但是s并且foo保留在模块的专用符号表中，并且在本地上下文中没有意义：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>s
...
</code></pre><p>要在本地上下文中访问，模块中定义的对象名称必须加上mod前缀:</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>mod<span class="token punctuation">.</span>s
...
[100, 200, 300]
</code></pre><p>它甚至可以不加选择地从一个模块导入所有内容:</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>from &lt;module_name&gt; import *
...
mod.Foo
</code></pre><p>在大规模生产代码中并不推荐这样做。这有点危险，因为您是在将名称全部输入到本地符号表中。除非您对它们都很了解，并且确信不会发生冲突，否则您很有可能会不小心覆盖现有的名称。但是，当您只是为了测试或发现的目的而随意使用交互式解释器时，这种语法非常方便，因为它可以让您快速访问模块必须提供的所有内容，而无需大量输入。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>**from &lt;module_name&gt; import &lt;name&gt; as &lt;alt_name&gt;**
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>也可以导入单独的对象，但是使用替代名称将它们输入到本地符号表中:</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>from &lt;module_name&gt; import &lt;name&gt; as &lt;alt_name&gt;[, &lt;name&gt; as &lt;alt_name&gt; …]
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>这样就可以将名称直接放置在本地符号表中，但可以避免与以前存在的名称冲突：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>s <span class="token operator">=</span> <span class="token string">&#39;foo&#39;</span>
...
[100, 200, 300]
</code></pre><p><strong>import &lt;module_name&gt; as &lt;alt_name&gt;</strong></p><p>您也可以使用备用名称导入整个模块：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>import &lt;module_name&gt; as &lt;alt_name&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> mod <span class="token keyword">as</span> my_module
...
arg = qux
</code></pre><p>可以从函数定义中导入模块内容。在这种情况下，import只有在调用该函数后，才会发生：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">def</span> <span class="token function">bar</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span>
...
</code></pre><p>最后，一个带有except ImportError子句的try语句可以用来防止不成功的导入尝试:</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">try</span><span class="token punctuation">:</span>
...
</code></pre><h3 id="_2-3-dir-函数" tabindex="-1"><a class="header-anchor" href="#_2-3-dir-函数"><span>2.3 dir()函数</span></a></h3><p>内置函数dir()返回一个名称空间中定义的名称列表。如果没有参数，它会在当前本地符号表中产生一个按字母顺序排序的名称列表:</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token builtin">dir</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">[</span><span class="token number">0</span><span class="token punctuation">:</span><span class="token number">5</span><span class="token punctuation">]</span>
...
</code></pre><p>注意dir()上面的第一个调用是如何列出几个自动定义的名称的，这些名称在解释器启动时已经存在于名称空间中。随着新的名称定义（qux），它们出现在的后续，调用dir()。这对于识别由import语句确切添加到名称空间的内容很有用。当给定参数作为模块名称时，dir()列出模块中定义的名称：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> mod
<span class="token builtin">dir</span><span class="token punctuation">(</span>mod<span class="token punctuation">)</span>
 ...
 &#39;foo&#39;,
 &#39;s&#39;]
</code></pre><h3 id="_2-4-将模块作为脚本执行" tabindex="-1"><a class="header-anchor" href="#_2-4-将模块作为脚本执行"><span>2.4 将模块作为脚本执行</span></a></h3><p>任何.py包含模块的文件本质上也是Python 脚本，没有任何理由它不能像一个脚本一样执行。 这里还是mod.py，正如上面定义的那样。比如命令行：</p><blockquote><p>python mod.py</p></blockquote><p>此外在mod.py文件中添加输出内容，如下所示:</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>s = &quot;Hello world!&quot;
a = [100, 200, 300]
...
x = Foo()
print(x)     
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>现在输出如下：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>Hello world!
[100, 200, 300]
arg = quux
&lt;__main__.Foo object at 0x7f1e792b9e50&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>不幸的是，现在当作为模块导入时，它还会生成输出：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>import mod
Hello world!
[100, 200, 300]
arg = quux
&lt;mod.Foo object at 0x7f7db3aba310&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这可能不是您想要的。导入模块时，模块通常不生成输出。如果您可以区分文件是作为模块加载时还是作为独立脚本运行时，这不是很好吗？</p><p>将.py文件导入为模块时，Python 会将特殊的dunder变量设置为模块__name__的名称。但是，如果文件作为独立脚本运行，__name__则（创造性地）设置为string &#39;__main__&#39;。利用这个事实，您可以识别出运行时是哪种情况，并相应地更改mod.py为：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>s = &quot;Hello world!&quot;
a = [100, 200, 300]
...
if (__name__ == &#39;__main__&#39;):
    ...
    x = Foo()
    print(x)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2-5-重新加载模块" tabindex="-1"><a class="header-anchor" href="#_2-5-重新加载模块"><span>2.5 重新加载模块</span></a></h3><p>为了提高效率，每个解释器会话仅加载一次模块。对于函数和类定义来说，这很好，它们通常占模块内容的大部分。但是一个模块也可以包含可执行语句，通常用于初始化。请注意，这些语句仅在第一次导入模块时执行。</p><p>考虑以下文件mod.py：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>a = [100, 200, 300]
print(&#39;a =&#39;, a)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>调用mod模块会以下结果：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>&gt;&gt;&gt; import mod
a = [100, 200, 300]
&gt;&gt;&gt; import mod
&gt;&gt;&gt; import mod
...
&gt;&gt;&gt; mod.a
[100, 200, 300]
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>该print()语句不会在后续导入上执行。（就此而言，赋值语句也不是，而是作为mod.ashows 值的最终显示，这无关紧要。完成赋值后，它会保留下来。）如果对模块进行了更改并需要重新加载，则需要重新启动解释器或使用reload()从module 调用的函数importlib：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>&gt;&gt;&gt; import mod
a = [100, 200, 300]
...
a = [100, 200, 300]
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_3-python包" tabindex="-1"><a class="header-anchor" href="#_3-python包"><span>3 Python包</span></a></h2><h3 id="_3-1-python包的使用" tabindex="-1"><a class="header-anchor" href="#_3-1-python包的使用"><span>3.1 Python包的使用</span></a></h3><p>假设您开发了一个非常大的应用程序，其中包含许多模块。随着模块数量的增加，如果将它们倾倒到一个位置，则很难跟踪所有模块。如果它们具有相似的名称或功能，则尤其如此。您可能希望有一种分组和组织的方法。 包允许使用点表示法对模块名称空间进行分层结构。就像模块帮助避免全局变量名之间的冲突一样，包帮助避免模块名之间的冲突。</p><p>创建软件包非常简单，因为它利用了操作系统固有的分层文件结构。考虑以下结构目录安排：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>- work
  - mod1.py
  - mod2.py
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>mod1.py：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>def foo():
    print(&#39;[mod1] foo()&#39;)

class Foo:
    pass
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>mod2.py:</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>def bar():
    print(&#39;[mod2] bar()&#39;)

class Bar:
    pass
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>基于这种结构，如果mod1.py和mod2.py为workd文件夹下，你可以使用点符号引用这两个模块(work.mod1, work.mod2)，并使用你已经熟悉的语法导入它们:</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>import &lt;module_name&gt;[, &lt;module_name&gt; ...]
from &lt;module_name&gt; import &lt;name(s)&gt;
from &lt;module_name&gt; import &lt;name&gt; as &lt;alt_name&gt;
from &lt;package_name&gt; import &lt;modules_name&gt;[, &lt;module_name&gt; ...]
from &lt;package_name&gt; import &lt;module_name&gt; as &lt;alt_name&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> work<span class="token punctuation">.</span>mod1<span class="token punctuation">,</span> work<span class="token punctuation">.</span>mod2
...
</code></pre><p>也可以直接导入包</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> work
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>但这没有用。尽管严格来说，这是语法上正确的Python语句，但它并没有做任何有用的事情。特别是，它不会将work任何模块放入本地名称空间中：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>&gt;&gt;&gt; import work
&gt;&gt;&gt; work.mod1
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>要实际导入模块或其内容，您需要使用上面显示的形式之一。</p><h3 id="_3-2-包初始化" tabindex="-1"><a class="header-anchor" href="#_3-2-包初始化"><span>3.2 包初始化</span></a></h3><p>如果包目录中存在一个名为__init__.py的文件，则在导入包或包中的模块时将调用该文件。这可用于执行程序包初始化代码，例如程序包级数据的初始化。 例如，考虑以下__init__.py文件：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>print(f&#39;Invoking __init__.py for {__name__}&#39;)
A = [&#39;quux&#39;, &#39;corge&#39;, &#39;grault&#39;]
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>让我们从上面的例子将这个文件添加到work目录:</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>- work
  - __init__.py
  - mod1.py
  - mod2.py
...
[&#39;quux&#39;, &#39;corge&#39;, &#39;grault&#39;]
</code></pre><p>包中的模块可以通过依次导入全局变量来访问全局变量，让我们修改mod1.py</p><p>mod1.py</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>def foo():
    from work import A
    print(&#39;[mod1] foo() / A = &#39;, A)

class Foo:
    pass
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">from</span> work <span class="token keyword">import</span> mod1
...
</code></pre><p>__init__.py也可以用于从软件包中自动导入模块。例如，在前面您已经看到，该语句import work仅将名称work放置在调用者的本地符号表中，而不会导入任何模块。但是，如果__init__.py在work目录中包含以下内容：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>print(f&#39;Invoking __init__.py for {__name__}&#39;)
import work.mod1, work.mod2
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>模块mod1和mod2会自动导入</p><h3 id="_3-3-从包中导入" tabindex="-1"><a class="header-anchor" href="#_3-3-从包中导入"><span>3.3 从包中导入</span></a></h3><p>出于以下讨论的目的，先前定义的程序包已扩展为包含一些其他模块：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>- work
  - mod1.py
  - mod2.py
  - mod3.py
  - mod4.py
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>现在，work目录中定义了四个模块。它们的内容如下所示：</p><p>mod1.py</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>def foo():
    print(&#39;[mod1] foo()&#39;)
...
class Bar:
    pass
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>mod3.py</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>def baz():
    print(&#39;[mod3] baz()&#39;)

class Baz:
    pass
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>mod4.py</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>def qux():
    print(&#39;[mod4] qux()&#39;)

class Qux:
    pass
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>您已经看到，当import *用于模块时，模块中的所有对象都将导入到本地符号表中，除了那些名称以下划线开头的对象外，与往常一样。ython遵循以下约定：如果package目录中的__init__.py文件包含名为__all__的列表，则这个列表中视为要导入的模块。</p><p>对于本示例，假设您在work录中创建一个__init__.py：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>__all__ = [
        &#39;mod1&#39;,
        &#39;mod2&#39;,
        &#39;mod3&#39;,
        &#39;mod4&#39;
...
</code></pre><p>顺便说一句，__all__也可以在模块中定义它，并且具有相同的目的：控制使用导入的内容import *。例如，修改mod1.py如下：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>__all__ = [&#39;foo&#39;]
...
class Foo:
    pass
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>from work.mod1 import *只会导入__all__中的内容</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>from work.mod1 import *
&gt;&gt;&gt; foo()
[mod1] foo()
&gt;&gt;&gt; Foo
Traceback (most recent call last):
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>总之，包和模块__all__都使用它来控制在指定时导入的内容。但是默认行为不同：import *</p><ul><li>对于包，当__all__未定义时，import *不会导入任何内容。</li><li>对于__all__未定义的模块，将import *导入所有内容（除非您猜对了，否则名称会以下划线开头）</li></ul><h3 id="_3-4-子包" tabindex="-1"><a class="header-anchor" href="#_3-4-子包"><span>3.4 子包</span></a></h3><p>程序包可以包含嵌套子程序包到任意深度。例如，让我们对示例包目录进行如下修改：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>- work
  - subpackage1
    - mod1.py
    - mod2.py
  - subpackage2
...
[mod3] baz()
[mod4] qux()
</code></pre><p>此外，一个子包中的模块可以引用同级子包中的对象(如果同级包中包含您需要的某些功能)。例如，假设您希望从模块mod3中导入和执行函数foo()(在模块mod1中定义)。你可以使用绝对导入:</p><p>work/subpackage2/mod3.py中的内容如下：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>def baz():
    print(&#39;[mod3] baz()&#39;)
