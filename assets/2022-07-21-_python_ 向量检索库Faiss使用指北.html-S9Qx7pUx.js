import{_ as e,r as o,o as c,c as i,a as n,b as s,d as t,e as p}from"./app-CD_-ggd_.js";const l={},u=n("h1",{id:"python-向量检索库faiss使用指北",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#python-向量检索库faiss使用指北"},[n("span",null,"[python] 向量检索库Faiss使用指北")])],-1),r={href:"https://github.com/facebookresearch/faiss",target:"_blank",rel:"noopener noreferrer"},d={href:"https://github.com/facebookresearch/faiss/wiki",target:"_blank",rel:"noopener noreferrer"},k=p(`<p>对于Faiss，linux系统的安装如下：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code># CPU安装
pip install faiss-cpu
# GPU安装
pip install faiss-gpu
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>windows系统的安装需要conda，安装指令如下。使用前需要conda activate激活环境，不要直接用pip安装。</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code># CPU安装
conda install -c pytorch faiss-cpu

# GPU安装
conda install -c pytorch faiss-gpu
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_1-基础使用" tabindex="-1"><a class="header-anchor" href="#_1-基础使用"><span>1 基础使用</span></a></h2><h3 id="_1-1-入门" tabindex="-1"><a class="header-anchor" href="#_1-1-入门"><span>1.1 入门</span></a></h3><p><strong>数据库的建立</strong></p><p>Faiss可以处理固定维度的向量集合，这些集合可以存储在矩阵中。Faiss仅使用32位浮点矩阵，其中矩阵的列表示向量的特征，行表示向量样本数。一般来说进行向量检索，我们需要两个矩阵：</p><ul><li>索引矩阵，索引矩阵包含所有必须索引的向量，我们将在其中搜索。其维度为[nb向量样本数,d向量维度]。</li><li>查询矩阵，查询矩阵。它的大小是[nq向量样本数,d向量维度]。如果我们只有一个查询向量，则nq=1。</li></ul><p>下面例子介绍了faiss所需输入矩阵的例子。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> numpy <span class="token keyword">as</span> np
...
[[ 842  781 1939 1535 2579]
 ...
 [ 852  211  673  937  228]]
</code></pre><p><strong>上面的程序用numpy直接的计算方式如下，如果维度不高，样本不多，速度要比faiss快。但是如果数据很复杂，以下的程序不要运行，会导致系统崩溃的。</strong></p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>
...
<span class="token comment"># 最后五个查询向量的检索结果</span>
<span class="token comment"># print(I[-5:])</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_1-2-加快检索速度" tabindex="-1"><a class="header-anchor" href="#_1-2-加快检索速度"><span>1.2 加快检索速度</span></a></h3><p>前面的代码可以看到，检索速度太慢，尤其是在很弱性能的电脑上。为了加快检索速度，在Faiss中，将数据集分割成若干部分，然后在d维空间中定义了Voronoi单元，每个数据库向量都落在其中一个单元中。这个就是基于Voronoi图的空间最近邻近查询。在搜索时，查询向量先判断落在哪个单元上，然后将其与所在的单元中的索引向量和几个相邻单元的索引向量进行比较。</p><p>在Faiss中通过设置IndexIVFFlat索引方式完成，但是这种类型的索引需要一个训练阶段，而且可以在与数据库向量具有相同分布的任何向量集合上执行。IndexIVFFlat还需要另一个索引量化器，通过该量化器将各个索引向量分配给Voronoi单元，这一操作一般通过使用IndexFlatL2来实现。这种方式建库耗时，用于检索的相邻单元数量越少，检索速度越快，精度越低。需要实际情况平衡精度和速度设置用于检索的相邻单元数量。</p><p>在IndexIVFFlat搜索方法有两个参数：用于设置单元格数量的nlist，和设置在多少个相近单元进行查找nprobe（默认为一个）。 nprob的增加会导致检索速度线性增加，但是检索精度也会随之上升。一个检索示例如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 单元数</span>
...
True
</code></pre><p><strong>在1个相近单元上进行查找</strong></p><p>可以看到检索速度变快很多，但是精度会下降。但是与前面的L2检索结果不完全相同。这是因为某些结果不在带检索的Voronoi单元中。因此，检索时访问更多的单元可能会被证明是有用的。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 默认1个相近单元用于查找</span>
...
[[ 937 1026  879  461  313]
 ...
 [ 211  228  412  267   35]]
</code></pre><p><strong>在多个相近单元上进行查找</strong></p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 设置在多少个相近单元进行查找</span>
...
[[ 842  781 1939 1535 1951]
 ...
 [ 852  211  673  937  228]]
</code></pre><p><strong>在所有相近单元上进行查找</strong></p><p>当nprobe=nlist，表示在所有的数据单元上进行查找，这种方式会给出给出与直接L2检索相同的结果，但速度很慢。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 设置在多少个相近单元进行查找</span>
...
[[ 842  781 1939 1535 2579]
 ...
 [1403  373 1032  862 1975]
 [ 852  211  673  937  228]]
</code></pre><h3 id="_1-3-更低的内存占用" tabindex="-1"><a class="header-anchor" href="#_1-3-更低的内存占用"><span>1.3 更低的内存占用</span></a></h3><p>我们看到的索引IndexFlatL2和IndexIVFFlat都存储完整向量。为了扩展到超大数据集，Faiss提供了各种变体，使用基于乘积量化器的有损压缩来压缩存储的向量。向量仍然存储在Voronoi 单元中，但它们的大小减少到可配置的字节数m（特征维度d必须是m的倍数）。</p><p>压缩基于乘积量化器，可以看作是额外的量化级别，应用于要编码的向量的子向量。在这种情况下，由于向量没有精确存储，因此搜索方法返回的距离也是近似值。但是这种方法精度不高，而且训练模型时间很长，谨慎使用。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>nlist <span class="token operator">=</span> <span class="token number">100</span>
...
</code></pre><p>数据归一化后，计算内积，代码如下。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 加载faiss库</span>
<span class="token keyword">import</span> faiss            
<span class="token comment"># 设置检索维度       </span>
index <span class="token operator">=</span> faiss<span class="token punctuation">.</span>IndexFlatIP<span class="token punctuation">(</span>d<span class="token punctuation">)</span>   
<span class="token comment"># 判断该索引方式是否已经训练，false表示没有训练</span>
...
</code></pre><p>可以numpy直接计算归一化，计算内积，推荐这种方式，代码如下。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 加载faiss库</span>
<span class="token keyword">import</span> faiss            
<span class="token comment"># 设置检索维度       </span>
index <span class="token operator">=</span> faiss<span class="token punctuation">.</span>IndexFlatIP<span class="token punctuation">(</span>d<span class="token punctuation">)</span>   
<span class="token comment"># 判断该索引方式是否已经训练，false表示没有训练</span>
...
</code></pre>`,58),m={href:"https://gist.github.com/mdouze/6cc12fa967e5d9911580ef633e559476",target:"_blank",rel:"noopener noreferrer"},v={href:"https://zhuanlan.zhihu.com/p/46626607",target:"_blank",rel:"noopener noreferrer"},b=p(`<h3 id="_2-2-faiss-构建模块" tabindex="-1"><a class="header-anchor" href="#_2-2-faiss-构建模块"><span>2.2 Faiss 构建模块</span></a></h3><p>Faiss 建立在一些具有非常高效实现的基本算法之上：k-means 聚类、PCA、PQ 编码/解码。</p><p><strong>聚类</strong></p><p>Faiss提供了一种高效的k-means实现。如下所示。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">from</span> sklearn<span class="token punctuation">.</span>datasets <span class="token keyword">import</span> make_blobs
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 查看数据</span>
...
(1000,)
</code></pre><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 簇中心</span>
...
0.6
</code></pre><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># sklearn聚类</span>
...
</code></pre><p>一般来说当样本维度过高，或数据量大的时候，faiss进行聚类是个很不错的选择。</p><p><strong>降维</strong></p><p>faiss降维主要使用PCA，速度比sklearn快得多。示例代码如下</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 随机创建数据</span>
<span class="token comment"># n_features特征维度，n_samples样本数，centers聚类中心也可以理解为label数，cluster每个类别数据的方差，random_state随机种子</span>
...
data <span class="token operator">=</span> data<span class="token punctuation">.</span>astype<span class="token punctuation">(</span><span class="token string">&#39;float32&#39;</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 降到2维</span>
...
</code></pre><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/Face/[python] 向量检索库Faiss使用指北/output_39_1.png" alt="png" tabindex="0" loading="lazy"><figcaption>png</figcaption></figure><p><strong>乘积量化</strong></p>`,22),h={href:"https://blog.csdn.net/tiangcs/article/details/124012169",target:"_blank",rel:"noopener noreferrer"},y=p(`<div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 数据维度</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 编码</span>
...
</code></pre>`,3),g={href:"https://www.cnblogs.com/yhzhou/p/10568728.html",target:"_blank",rel:"noopener noreferrer"},_=n("h2",{id:"_3-参考",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#_3-参考"},[n("span",null,"3 参考")])],-1),x={href:"https://github.com/facebookresearch/faiss",target:"_blank",rel:"noopener noreferrer"},f={href:"https://github.com/facebookresearch/faiss/wiki",target:"_blank",rel:"noopener noreferrer"},F={href:"https://zhuanlan.zhihu.com/p/46626607",target:"_blank",rel:"noopener noreferrer"},w={href:"https://gist.github.com/mdouze/6cc12fa967e5d9911580ef633e559476",target:"_blank",rel:"noopener noreferrer"},q={href:"https://blog.csdn.net/tiangcs/article/details/124012169",target:"_blank",rel:"noopener noreferrer"},I={href:"https://www.cnblogs.com/yhzhou/p/10568728.html",target:"_blank",rel:"noopener noreferrer"};function E(P,A){const a=o("ExternalLinkIcon");return c(),i("div",null,[u,n("p",null,[s("Faiss是一个由facebook开发以用于高效相似性搜索和密集向量聚类的库。它能够在任意大小的向量集中进行搜索。它还包含用于评估和参数调整的支持代码。Faiss是用C++编写的，带有Python的完整接口。一些最有用的算法是在GPU上实现的。Faiss官方仓库为:"),n("a",r,[s("faiss"),t(a)]),s("。")]),n("p",null,[s("所谓相似性搜索是指通过比较多维空间中数据之间的相似性来搜索与输入数据最相似的目标数据。例如人脸识别中，通过比较人脸向量之前的距离来识别当前人脸与哪张人脸相似。因此，该技术被广泛应用于信息检索、计算机视觉、数据分析等领域。如果要检索的数据很多时，那么就需要一个向量检索库来加速检索。Faiss包含多种相似性搜索方法，并提供cpu和gpu版本支持。Faiss的优势在于通过较小的精度损失提高向量相似度的检索速度和减少内存使用量。本文主要讲述faiss的python3接口使用。faiss官方教程见："),n("a",d,[s("faiss官方教程"),t(a)]),s("。")]),k,n("p",null,[s("此外还有一种常用的距离：马氏距离Mahalanobis，faiss中马氏距离的用法见"),n("a",m,[s("mahalnobis_to_L2.ipynb"),t(a)]),s("。关于马氏距离的说明见"),n("a",v,[s("马氏距离(Mahalanobis Distance)"),t(a)]),s("。")]),b,n("p",null,[s("有时原始向量维度或向量数太多太高，计算相似度太慢。通过乘积量化对原始向量进行压缩，降低向量维度，压缩数据信息加快检索速度同时节省了内存空间。关于乘积量化见"),n("a",h,[s("ANN之乘积量化PQ"),t(a)]),s("。此外压缩后的数据可以进行恢复为原数据，一般重建误差不高，但建立量化器速度很慢。")]),y,n("p",null,[n("strong",null,[s("关于faiss中其他接口的介绍和使用见"),n("a",g,[s("Faiss流程与原理分析"),t(a)]),s("。")])]),_,n("ul",null,[n("li",null,[n("a",x,[s("faiss"),t(a)])]),n("li",null,[n("a",f,[s("faiss官方教程"),t(a)])]),n("li",null,[n("a",F,[s("马氏距离(Mahalanobis Distance)"),t(a)])]),n("li",null,[n("a",w,[s("mahalnobis_to_L2.ipynb"),t(a)])]),n("li",null,[n("a",q,[s("ANN之乘积量化PQ"),t(a)])]),n("li",null,[n("a",I,[s("Faiss流程与原理分析"),t(a)])])])])}const B=e(l,[["render",E],["__file","2022-07-21-_python_ 向量检索库Faiss使用指北.html.vue"]]),C=JSON.parse('{"path":"/blog/python/python%E5%AD%A6%E4%B9%A0/2022-07-21-_python_%20%E5%90%91%E9%87%8F%E6%A3%80%E7%B4%A2%E5%BA%93Faiss%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97.html","title":"[python] 向量检索库Faiss使用指北","lang":"zh-CN","frontmatter":{"date":"2022-07-21T11:42:08.000Z","category":["Python"],"tag":["Python","机器学习","深度学习"],"description":"[python] 向量检索库Faiss使用指北 Faiss是一个由facebook开发以用于高效相似性搜索和密集向量聚类的库。它能够在任意大小的向量集中进行搜索。它还包含用于评估和参数调整的支持代码。Faiss是用C++编写的，带有Python的完整接口。一些最有用的算法是在GPU上实现的。Faiss官方仓库为:faiss。 所谓相似性搜索是指通过比较...","head":[["meta",{"property":"og:url","content":"https://luohenyueji.github.io/blog/python/python%E5%AD%A6%E4%B9%A0/2022-07-21-_python_%20%E5%90%91%E9%87%8F%E6%A3%80%E7%B4%A2%E5%BA%93Faiss%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97.html"}],["meta",{"property":"og:site_name","content":"落痕月极的博客"}],["meta",{"property":"og:title","content":"[python] 向量检索库Faiss使用指北"}],["meta",{"property":"og:description","content":"[python] 向量检索库Faiss使用指北 Faiss是一个由facebook开发以用于高效相似性搜索和密集向量聚类的库。它能够在任意大小的向量集中进行搜索。它还包含用于评估和参数调整的支持代码。Faiss是用C++编写的，带有Python的完整接口。一些最有用的算法是在GPU上实现的。Faiss官方仓库为:faiss。 所谓相似性搜索是指通过比较..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/Face/%5Bpython%5D%20%E5%90%91%E9%87%8F%E6%A3%80%E7%B4%A2%E5%BA%93Faiss%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/output_38_1.png"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"name":"twitter:card","content":"summary_large_image"}],["meta",{"name":"twitter:image:alt","content":"[python] 向量检索库Faiss使用指北"}],["meta",{"property":"article:author","content":"落痕月极"}],["meta",{"property":"article:tag","content":"Python"}],["meta",{"property":"article:tag","content":"机器学习"}],["meta",{"property":"article:tag","content":"深度学习"}],["meta",{"property":"article:published_time","content":"2022-07-21T11:42:08.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"[python] 向量检索库Faiss使用指北\\",\\"image\\":[\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/Face/%5Bpython%5D%20%E5%90%91%E9%87%8F%E6%A3%80%E7%B4%A2%E5%BA%93Faiss%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/output_38_1.png\\",\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/Face/%5Bpython%5D%20%E5%90%91%E9%87%8F%E6%A3%80%E7%B4%A2%E5%BA%93Faiss%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/output_39_1.png\\"],\\"datePublished\\":\\"2022-07-21T11:42:08.000Z\\",\\"dateModified\\":null,\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"落痕月极\\",\\"url\\":\\"/\\"}]}"]]},"headers":[{"level":2,"title":"1 基础使用","slug":"_1-基础使用","link":"#_1-基础使用","children":[{"level":3,"title":"1.1 入门","slug":"_1-1-入门","link":"#_1-1-入门","children":[]},{"level":3,"title":"1.2 加快检索速度","slug":"_1-2-加快检索速度","link":"#_1-2-加快检索速度","children":[]},{"level":3,"title":"1.3 更低的内存占用","slug":"_1-3-更低的内存占用","link":"#_1-3-更低的内存占用","children":[]}]},{"level":2,"title":"2 构建指标","slug":"_2-构建指标","link":"#_2-构建指标","children":[{"level":3,"title":"2.1 度量指标","slug":"_2-1-度量指标","link":"#_2-1-度量指标","children":[]},{"level":3,"title":"2.2 Faiss 构建模块","slug":"_2-2-faiss-构建模块","link":"#_2-2-faiss-构建模块","children":[]}]},{"level":2,"title":"3 参考","slug":"_3-参考","link":"#_3-参考","children":[]}],"git":{},"readingTime":{"minutes":15.27,"words":4582},"filePathRelative":"blog/python/python学习/2022-07-21-[python] 向量检索库Faiss使用指北.md","localizedDate":"2022年7月21日","excerpt":"\\n<p>Faiss是一个由facebook开发以用于高效相似性搜索和密集向量聚类的库。它能够在任意大小的向量集中进行搜索。它还包含用于评估和参数调整的支持代码。Faiss是用C++编写的，带有Python的完整接口。一些最有用的算法是在GPU上实现的。Faiss官方仓库为:<a href=\\"https://github.com/facebookresearch/faiss\\" target=\\"_blank\\" rel=\\"noopener noreferrer\\">faiss</a>。</p>\\n<p>所谓相似性搜索是指通过比较多维空间中数据之间的相似性来搜索与输入数据最相似的目标数据。例如人脸识别中，通过比较人脸向量之前的距离来识别当前人脸与哪张人脸相似。因此，该技术被广泛应用于信息检索、计算机视觉、数据分析等领域。如果要检索的数据很多时，那么就需要一个向量检索库来加速检索。Faiss包含多种相似性搜索方法，并提供cpu和gpu版本支持。Faiss的优势在于通过较小的精度损失提高向量相似度的检索速度和减少内存使用量。本文主要讲述faiss的python3接口使用。faiss官方教程见：<a href=\\"https://github.com/facebookresearch/faiss/wiki\\" target=\\"_blank\\" rel=\\"noopener noreferrer\\">faiss官方教程</a>。</p>","autoDesc":true}');export{B as comp,C as data};
