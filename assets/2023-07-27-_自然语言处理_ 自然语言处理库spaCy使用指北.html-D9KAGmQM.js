...
</code></pre><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> spacy
...
1
billion
</code></pre><p>对于中文分词有时会出现专有名词被拆分，比如南京长江大桥被拆分为南京、长江、大桥。我们可以添加自定义词典来解决该问题，但是要注意的是自定义词典添加只针对某些语言模型。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> spacy
...
之一
！
</code></pre><h3 id="_2-2-词性标注与依存关系" tabindex="-1"><a class="header-anchor" href="#_2-2-词性标注与依存关系"><span>2.2 词性标注与依存关系</span></a></h3><p>spaCy在分词后，会对句子中每个词进行词性标注以及确定句子中单词之间的语法关系，要注意这些关系具体范围取决于所使用的模型。示例代码如下所示：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> spacy
...
</code></pre><p>在上述代码中pos_所用是常见的单词词性标注。tag_所支持的词性标注及解释如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">## 获取所有词性标注（tag_）和它们的描述</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>dep__所支持的依存关系及解释如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">## 获取所有依存关系标注和它们的描述</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2-3-命名实体识别" tabindex="-1"><a class="header-anchor" href="#_2-3-命名实体识别"><span>2.3 命名实体识别</span></a></h3><p>命名实体识别(Named Entity Recognition, 简称NER)是自然语言处理中的一项基础任务，应用范围非常广泛。 NER是指识别文本中具有特定意义或者指代性强的实体，通常包括人名、地名、机构名、日期时间、专有名词等。spaCy使用如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> spacy
...
1991年 24 29 DATE
468米 36 40 QUANTITY
</code></pre><p>如果想知道所有的实体以及其对应的含义，可以执行以下代码：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">## 获取命名实体标签及其含义</span>
...
</code></pre><h3 id="_2-4-词向量与相似性" tabindex="-1"><a class="header-anchor" href="#_2-4-词向量与相似性"><span>2.4 词向量与相似性</span></a></h3><p>词向量是自然语言处理中一种重要的表示方式，它将单词映射为实数向量。这种表示方式能够捕捉单词之间的语义关系，并将语义信息转化为计算机能够处理的数值形式。</p><p>传统的自然语言处理方法往往将文本表示为离散的符号，例如独热编码或者词袋模型。然而，这种方法忽略了单词之间的语义相似性，而且维度过高，造成稀疏性问题。相比之下，词向量通过将每个单词映射到连续的向量空间中，可以更好地捕捉单词之间的语义关系，并且降低了特征空间的维度，使得文本处理更加高效。通过计算两个词向量之间的距离或夹角可以衡量词向量的相似性。</p><p>提取句子中每一个词的词向量代码如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> spacy
...
！ True 12.811246 True
</code></pre><p>如果想得到某个句子或者某个词的词向量，代码如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">## 该词向量没有归一化</span>
...
</code></pre><p>spaCy提供了similarity函数以计算两个文本向量的相似度。 示例代码如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> spacy
...
</code></pre>`,37),C={href:"https://github.com/explosion/sense2vec",target:"_blank",rel:"noopener noreferrer"},x=e(`<p>此外如果仅仅使用spacy提取文本向量，可以用numpy手动计算文本相似度，代码如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> numpy <span class="token keyword">as</span> np
<span class="token keyword">import</span> spacy
nlp <span class="token operator">=</span> spacy<span class="token punctuation">.</span>load<span class="token punctuation">(</span><span class="token string">&quot;zh_core_web_sm&quot;</span><span class="token punctuation">)</span>  
doc1 <span class="token operator">=</span> nlp<span class="token punctuation">(</span><span class="token string">&quot;东方明珠是一座位于中国上海市的标志性建筑&quot;</span><span class="token punctuation">)</span>
doc2 <span class="token operator">=</span> nlp<span class="token punctuation">(</span><span class="token string">&quot;南京长江大桥是金陵四十景之一！&quot;</span><span class="token punctuation">)</span>
...
</code></pre><p>基于以下代码可以控制组件的选择和使用，以加快执行速度：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">## 加载不包含命名实体识别器（NER）的管道</span>
...
</code></pre><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">## 只启用tagger管道</span>
...
</code></pre><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment">## 加载词性标注器（tagger）和依存句法解析器（parser），但不启用它们</span>
...
</code></pre><p><strong>Example</strong></p>`,30),w={href:"https://spacy.io/usage/training",target:"_blank",rel:"noopener noreferrer"},q=e(`<p><strong>Language</strong></p><p>Language是spaCy的核心对象之一，它负责处理文本的预处理、词性标注、句法分析等任务。可以通过spacy.load()来加载一个具体的语言模型，获取对应的Language对象。</p><p><strong>Lexeme</strong></p><p>Lexeme 是一个单词在词汇表中的表示，它包含了关于该单词的各种信息，如词性、词频等。示例代码如下：</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code>nlp <span class="token operator">=</span> spacy<span class="token punctuation">.</span>load<span class="token punctuation">(</span><span class="token string">&quot;en_core_web_sm&quot;</span><span class="token punctuation">)</span>
...
</code></pre><h2 id="_4-参考" tabindex="-1"><a class="header-anchor" href="#_4-参考"><span>4 参考</span></a></h2>`,18),T={href:"https://github.com/explosion/spaCy",target:"_blank",rel:"noopener noreferrer"},N={href:"https://spacy.io",target:"_blank",rel:"noopener noreferrer"},D={href:"https://spacy.io/usage/models",target:"_blank",rel:"noopener noreferrer"},P={href:"https://github.com/huggingface/transformers",target:"_blank",rel:"noopener noreferrer"},B={href:"https://github.com/PaddlePaddle/PaddleNLP",target:"_blank",rel:"noopener noreferrer"},F={href:"https://github.com/nltk/nltk",target:"_blank",rel:"noopener noreferrer"},L={href:"https://spacy.io/usage",target:"_blank",rel:"noopener noreferrer"},z={href:"https://github.com/explosion/sense2vec",target:"_blank",rel:"noopener noreferrer"},R={href:"https://spacy.io/usage/training",target:"_blank",rel:"noopener noreferrer"};function S(O,j){const a=o("ExternalLinkIcon");return i(),c("div",null,[u,n("p",null,[s("spaCy是一个基于Python编写的开源自然语言处理库。基于自然处理领域的最新研究，spaCy提供了一系列高效且易用的工具，用于文本预处理、文本解析、命名实体识别、词性标注、句法分析和文本分类等任务。 spaCy的官方仓库地址为："),n("a",r,[s("spaCy-github"),t(a)]),s("。本文主要参考其官方网站的文档，spaCy的官方网站地址为："),n("a",d,[s("spaCy"),t(a)]),s("。")]),k,n("p",null,[s("在众多自然语言处理库中，spaCy库提供了超过73种语言的支持，并为25种语言提供了训练代码。该库提供了一系列简单易用的模型和函数接口，包括分词、词性标注等功能。用户还可以使用PyTorch、TensorFlow等框架在spaCy创建自定义模型，以满足特定需求。spaCy支持的语言模型见"),n("a",m,[s("spaCy-models"),t(a)]),s("。")]),n("p",null,[s("事实上，有一些自然语言处理开源库，例如HuggingFace的"),n("a",v,[s("Transformers"),t(a)]),s("、"),n("a",g,[s("PaddleNLP"),t(a)]),s("和"),n("a",y,[s("NLTK"),t(a)]),s("，相较于spaCy来说更为专业且性能更好。然而，对于简单的应用而言，spaCy更为适合，因为它具有简单易用、功能全面，同时也提供了大量面向多语言预训练模型的优点。此外，随着以GPT-3为代表的语言大模型在自然语言处理领域取得了巨大的突破和成功，原本一些自然语言处理库在精度上实际不如语言大模型。然而，使用语言大模型需要庞大的推理资源，而在对精度要求不高的场景中，使用spaCy这类小巧的自然语言处理库依然是很合适的选择。")]),b,h,n("p",null,[s("为了实现这一目标，spaCy提供了配置化的安装指令选择页面供用户使用。安装指令选择页面地址为"),n("a",_,[s("spaCy-usage"),t(a)]),s("。下图展示了本文的安装配置项，本文采用了最简单的cpu推理模式。")]),E,n("p",null,[s("该部分内容和图片主要来自于"),n("a",A,[s("spacy-101"),t(a)]),s("的总结。spaCy提供的主要函数模块分为以下模块，接下来分别对这些模块进行介绍。")]),f,n("p",null,[s("在上面代码中相似度计算方式默认使用余弦相似度。余弦相似度范围为0到1，数值越高表明词向量越相似。一般来说词向量相似度使用spacy通用模型准确度可能很低，可以尝试使用专用模型或者自行训练模型，spacy推荐使用"),n("a",C,[s("sense2vec"),t(a)]),s("来计算模型相似度。")]),x,n("p",null,[s("Example用于训练spaCy模型，它包含了一个输入文本(Doc)和其对应的标注数据。关于spaCy模型的训练，见:"),n("a",w,[s("spaCy-training"),t(a)])]),q,n("ul",null,[n("li",null,[n("a",T,[s("spaCy-github"),t(a)])]),n("li",null,[n("a",N,[s("spaCy"),t(a)])]),n("li",null,[n("a",D,[s("spaCy-models"),t(a)])]),n("li",null,[n("a",P,[s("Transformers"),t(a)])]),n("li",null,[n("a",B,[s("PaddleNLP"),t(a)])]),n("li",null,[n("a",F,[s("NLTK"),t(a)])]),n("li",null,[n("a",L,[s("spaCy-usage"),t(a)])]),n("li",null,[n("a",z,[s("sense2vec"),t(a)])]),n("li",null,[n("a",R,[s("spaCy-training"),t(a)])])])])}const V=p(l,[["render",S],["__file","2023-07-27-_自然语言处理_ 自然语言处理库spaCy使用指北.html.vue"]]),I=JSON.parse('{"path":"/blog/%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E4%B8%8E%E8%AF%AD%E9%9F%B3%E8%AF%86%E5%88%AB/2023-07-27-_%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86_%20%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E5%BA%93spaCy%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97.html","title":"[自然语言处理] 自然语言处理库spaCy使用指北","lang":"zh-CN","frontmatter":{"date":"2023-07-27T13:00:33.000Z","tag":["自然语言处理与语音识别","深度学习","Python"],"category":["自然语言处理与语音识别"],"description":"[自然语言处理] 自然语言处理库spaCy使用指北 spaCy是一个基于Python编写的开源自然语言处理库。基于自然处理领域的最新研究，spaCy提供了一系列高效且易用的工具，用于文本预处理、文本解析、命名实体识别、词性标注、句法分析和文本分类等任务。 spaCy的官方仓库地址为：spaCy-github。本文主要参考其官方网站的文档，spaCy的官...","head":[["meta",{"property":"og:url","content":"https://luohenyueji.github.io/blog/%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E4%B8%8E%E8%AF%AD%E9%9F%B3%E8%AF%86%E5%88%AB/2023-07-27-_%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86_%20%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E5%BA%93spaCy%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97.html"}],["meta",{"property":"og:site_name","content":"落痕月极的博客"}],["meta",{"property":"og:title","content":"[自然语言处理] 自然语言处理库spaCy使用指北"}],["meta",{"property":"og:description","content":"[自然语言处理] 自然语言处理库spaCy使用指北 spaCy是一个基于Python编写的开源自然语言处理库。基于自然处理领域的最新研究，spaCy提供了一系列高效且易用的工具，用于文本预处理、文本解析、命名实体识别、词性标注、句法分析和文本分类等任务。 spaCy的官方仓库地址为：spaCy-github。本文主要参考其官方网站的文档，spaCy的官..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/NLP/%5B%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%5D%20%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E5%BA%93spaCy%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/image/img1.png"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"name":"twitter:card","content":"summary_large_image"}],["meta",{"name":"twitter:image:alt","content":"[自然语言处理] 自然语言处理库spaCy使用指北"}],["meta",{"property":"article:author","content":"落痕月极"}],["meta",{"property":"article:tag","content":"自然语言处理与语音识别"}],["meta",{"property":"article:tag","content":"深度学习"}],["meta",{"property":"article:tag","content":"Python"}],["meta",{"property":"article:published_time","content":"2023-07-27T13:00:33.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"[自然语言处理] 自然语言处理库spaCy使用指北\\",\\"image\\":[\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/NLP/%5B%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%5D%20%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E5%BA%93spaCy%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/image/img1.png\\",\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/NLP/%5B%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%5D%20%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E5%BA%93spaCy%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/image/img2.png\\",\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/NLP/%5B%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%5D%20%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E5%BA%93spaCy%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/image/img3.svg\\",\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/NLP/%5B%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%5D%20%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E5%BA%93spaCy%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/image/img4.svg\\",\\"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/Python-Study-Notes/NLP/%5B%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%5D%20%E8%87%AA%E7%84%B6%E8%AF%AD%E8%A8%80%E5%A4%84%E7%90%86%E5%BA%93spaCy%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8C%97/image/img5.svg\\"],\\"datePublished\\":\\"2023-07-27T13:00:33.000Z\\",\\"dateModified\\":null,\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"落痕月极\\",\\"url\\":\\"/\\"}]}"]]},"headers":[{"level":2,"title":"1 背景介绍与spaCy安装","slug":"_1-背景介绍与spacy安装","link":"#_1-背景介绍与spacy安装","children":[{"level":3,"title":"1.1 自然语言处理简介","slug":"_1-1-自然语言处理简介","link":"#_1-1-自然语言处理简介","children":[]},{"level":3,"title":"1.2 spaCy安装","slug":"_1-2-spacy安装","link":"#_1-2-spacy安装","children":[]}]},{"level":2,"title":"2 spaCy快速入门","slug":"_2-spacy快速入门","link":"#_2-spacy快速入门","children":[{"level":3,"title":"2.1 分词","slug":"_2-1-分词","link":"#_2-1-分词","children":[]},{"level":3,"title":"2.2 词性标注与依存关系","slug":"_2-2-词性标注与依存关系","link":"#_2-2-词性标注与依存关系","children":[]},{"level":3,"title":"2.3 命名实体识别","slug":"_2-3-命名实体识别","link":"#_2-3-命名实体识别","children":[]},{"level":3,"title":"2.4 词向量与相似性","slug":"_2-4-词向量与相似性","link":"#_2-4-词向量与相似性","children":[]}]},{"level":2,"title":"3 spaCy结构体系","slug":"_3-spacy结构体系","link":"#_3-spacy结构体系","children":[{"level":3,"title":"3.1 spaCy处理流程","slug":"_3-1-spacy处理流程","link":"#_3-1-spacy处理流程","children":[]},{"level":3,"title":"3.2 spaCy工程结构","slug":"_3-2-spacy工程结构","link":"#_3-2-spacy工程结构","children":[]}]},{"level":2,"title":"4 参考","slug":"_4-参考","link":"#_4-参考","children":[]}],"git":{},"readingTime":{"minutes":19.53,"words":5859},"filePathRelative":"blog/自然语言处理与语音识别/2023-07-27-[自然语言处理] 自然语言处理库spaCy使用指北.md","localizedDate":"2023年7月27日","excerpt":"\\n<p>spaCy是一个基于Python编写的开源自然语言处理库。基于自然处理领域的最新研究，spaCy提供了一系列高效且易用的工具，用于文本预处理、文本解析、命名实体识别、词性标注、句法分析和文本分类等任务。\\nspaCy的官方仓库地址为：<a href=\\"https://github.com/explosion/spaCy\\" target=\\"_blank\\" rel=\\"noopener noreferrer\\">spaCy-github</a>。本文主要参考其官方网站的文档，spaCy的官方网站地址为：<a href=\\"https://spacy.io\\" target=\\"_blank\\" rel=\\"noopener noreferrer\\">spaCy</a>。</p>","autoDesc":true}');export{V as comp,I as data};
