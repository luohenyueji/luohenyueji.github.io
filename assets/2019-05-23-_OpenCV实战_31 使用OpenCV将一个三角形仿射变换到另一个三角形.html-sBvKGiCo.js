...
</code></pre><h3 id="_2-2-计算边界框" tabindex="-1"><a class="header-anchor" href="#_2-2-计算边界框" aria-hidden="true">#</a> 2.2 计算边界框</h3><p>在此步骤中，我们计算三角形周围的边界框。这个想法只是扭曲图像的一小部分而不是整个图像以提高效率。通过boundingRect()得到包覆此三角形的最小正矩形。代码如下：</p><p>C++:</p><pre><code>// Find bounding rectangle for each triangle
Rect r1 = boundingRect(tri1);
Rect r2 = boundingRect(tri2);
</code></pre><p>Python:</p><pre><code># Find bounding box. 
r1 = cv2.boundingRect(tri1)
r2 = cv2.boundingRect(tri2)
</code></pre><h3 id="_2-3-裁剪图像和更改坐标" tabindex="-1"><a class="header-anchor" href="#_2-3-裁剪图像和更改坐标" aria-hidden="true">#</a> 2.3 裁剪图像和更改坐标</h3><p>要有效地将仿射变换应用于图像而不是整个图像，我们将根据上一步中计算的边界框裁剪输入图像。还需要修改三角形的坐标以反映它们在新裁剪图像中的位置。这是通过从三角形的x和y坐标减去边界框左上角顶点的x和y坐标来完成的。代码如下：</p><p>C++:</p><pre><code>// Offset points by left top corner of the respective rectangles
...
</code></pre><h3 id="_2-4-计算仿射变换矩形" tabindex="-1"><a class="header-anchor" href="#_2-4-计算仿射变换矩形" aria-hidden="true">#</a> 2.4 计算仿射变换矩形</h3><p>我们刚刚在裁剪的输入和输出图像中获得了输入和输出三角形的坐标。使用这两个三角形，我们可以找到仿射变换，它将使用以下代码将输入三角形转换为裁剪图像中的输出三角形。</p><p>C++:</p><pre><code>// Given a pair of triangles, find the affine transform.
Mat warpMat = getAffineTransform( tri1Cropped, tri2Cropped );
</code></pre><p>Python:</p><pre><code># Given a pair of triangles, find the affine transform.
warpMat = cv2.getAffineTransform( np.float32(tri1Cropped), np.float32(tri2Cropped) )
</code></pre><h3 id="_2-5-应用仿射变换到三角形" tabindex="-1"><a class="header-anchor" href="#_2-5-应用仿射变换到三角形" aria-hidden="true">#</a> 2.5 应用仿射变换到三角形</h3><p>将上一步骤中找到的仿射变换矩阵应用于裁剪的输入图像，以获得裁剪的输出图像。在OpenCV中，您可以使用warpAffine将仿射变换应用于图像。代码如下：</p><p>C++:</p><pre><code>// Apply the Affine Transform just found to the src image
...
</code></pre><h3 id="_2-6-屏蔽三角形外的像素" tabindex="-1"><a class="header-anchor" href="#_2-6-屏蔽三角形外的像素" aria-hidden="true">#</a> 2.6 屏蔽三角形外的像素</h3><p>在上一步中，我们获得了输出矩形图像。但是，我们对矩形区域内的三角形感兴趣。因此，我们使用fillConvexPoly创建一个掩模，用于遮蔽三角形外的所有像素。这个新的裁剪图像最终可以使用输出边界矩形的左上角坐标点置于输出图像中的正确位置。</p><p>C++:</p><pre><code>// Get mask by filling triangle
...
</code></pre><h2 id="_3-代码" tabindex="-1"><a class="header-anchor" href="#_3-代码" aria-hidden="true">#</a> 3 代码</h2><p>本文所有代码见：</p>`,52),d={href:"https://github.com/luohenyueji/OpenCV-Practical-Exercise",target:"_blank",rel:"noopener noreferrer"},s=n("p",null,"C++:",-1),m=n("pre",null,[n("code",null,`#include "pch.h"
...
`)],-1),u=n("p",null,"Python:",-1),h=n("pre",null,[n("code",null,`#!/usr/bin/env python
...
if __name__ == '__main__' :
    ...
    cv2.waitKey(0)
`)],-1),f=n("h1",{id:"_4-参考",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#_4-参考","aria-hidden":"true"},"#"),t(" 4 参考")],-1),_={href:"https://www.learnopencv.com/warp-one-triangle-to-another-using-opencv-c-python/",target:"_blank",rel:"noopener noreferrer"};function C(y,b){const r=a("ExternalLinkIcon");return o(),p("div",null,[l,n("p",null,[n("a",d,[t(" https://github.com/luohenyueji/OpenCV-Practical-Exercise "),e(r)])]),s,m,u,h,f,n("ul",null,[n("li",null,[n("a",_,[t(" https://www.learnopencv.com/warp-one-triangle-to-another-using-opencv-c-python/ "),e(r)])])])])}const w=i(c,[["render",C],["__file","2019-05-23-_OpenCV实战_31 使用OpenCV将一个三角形仿射变换到另一个三角形.html.vue"]]);export{w as default};
