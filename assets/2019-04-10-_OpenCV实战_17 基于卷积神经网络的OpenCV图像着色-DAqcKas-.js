...
</code></pre><h3 id="_2-3-将图像转换为-cie-lab颜色空间" tabindex="-1"><a class="header-anchor" href="#_2-3-将图像转换为-cie-lab颜色空间"><span>2.3 将图像转换为 CIE Lab颜色空间</span></a></h3><p>缩放输入 RGB 图像，使得值在 0-1 范围内，然后将其转换为 Lab<br> 颜色空间，并提取出亮度通道。原始图像中的亮度通道的大小调整为网络输入大小，在这种情况下为（ 224,224 ）。通常，亮度通道的范围从 0 到100. 因此我们减去 50 以 0 点对称。然后我们将缩放和平均居中的亮度送到网络作为前向通道的输入。前向传递的输出是图像的预测 ab<br> 通道。将其缩放回原始图像大小，然后与原始大小的亮度图像（在原始分辨率中较早提取）合并，以获得输出 Lab 图像。然后将其转换为 RGB<br> 色彩空间以获得最终的彩色图像，并保存输出图像。</p><p>C++代码：</p><pre><code>	// extract L channel and subtract mean 将原图转换为Lab空间图，并归一化
	Mat lab, L, input;
...
print(&#39;Done !!!&#39;)
</code></pre><h2 id="_4-参考" tabindex="-1"><a class="header-anchor" href="#_4-参考"><span>4 参考</span></a></h2><ul><li><a href="https://www.learnopencv.com/convolutional-neural-network-based-image-colorization-using-opencv/" target="_blank" rel="noopener noreferrer"> https://www.learnopencv.com/convolutional-neural-network-based-image-<br> colorization-using-opencv/ </a></li></ul>`,100)]])}var s=r(a,[[`render`,o]]);export{i as _pageData,s as default};