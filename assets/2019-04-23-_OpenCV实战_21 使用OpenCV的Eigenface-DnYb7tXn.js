...
</code></pre><p>上面的代码执行以下操作。</p><p>1）将特征脸的数量（NUM_EIGEN_FACES）设置为10，将滑块的最大值（MAX_SLIDER_VALUE）设置为255.这些数字不是一成不变的。更改这些数字以查看应用程序的更改方式。</p><p>读取图像：接下来，我们使用函数readImages读取指定目录中的所有图像。该目录包含对齐的图像。所有图像中左眼和右眼的中心是相同的。我们将这些图像添加到列表（或矢量）中。我们还垂直翻转图像并将它们添加到列表中。因为并获取面部图像的镜像，我们3）创建图像数据矩阵。我们使用createDataMatrix函数将图像组合成数据矩阵。数据矩阵的每一行是一个图像。</p><p>4）计算PCA：接下来我们使用C ++中的PCA类和Python中的PCACompute函数计算PCA。作为PCA的输出，我们获得平均向量和10个特征向量。</p><p>5）重新设定图像维度以获得平均图像和特征图像：平均矢量和每个特征向量是长度为w * h *<br> 3的向量，其中w是宽度，h是高度，3是数据集中任何图像的颜色通道数。换句话说，它们是30k元素的向量。我们将它们重新塑造为图像的原始大小，以获得平均面部图像和EigenFaces。</p><p>6）调整权重获得不同的图像。在OpenCV中，滑块值不能为负数。所以我们通过从当前滑块值中减去MAX_SLIDER_VALUE /<br> 2来计算权重，这样我们就可以得到正值和负值。</p><p>结果如下图所示，左边是平均图像。在右边是一个新面孔，通过添加10个不同权重的特征面产生。在本文C++代码要添加一个dirent.h文件，自带代码就不列出来了，由于图像太少所以结果很模糊，加图像就行。不过opencv里面的pca计算很慢。</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]21 使用OpenCV的Eigenface/20190423194122118.jpeg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0x1b2hlbllK,size_16,color_FFFFFF,t_70" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><h2 id="_4-参考" tabindex="-1"><a class="header-anchor" href="#_4-参考"><span>4 参考</span></a></h2><ul><li><p><a href="https://www.learnopencv.com/eigenface-using-opencv-c-python/" target="_blank" rel="noopener noreferrer"> https://www.learnopencv.com/eigenface-using-opencv-c-python/</a></p></li><li><p><a href="https://www.learnopencv.com/principal-component-analysis/" target="_blank" rel="noopener noreferrer"> https://www.learnopencv.com/principal-component-analysis/</a></p></li></ul>`,93)]])}var s=r(a,[[`render`,o]]);export{i as _pageData,s as default};