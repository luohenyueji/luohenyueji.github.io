import{_ as i}from"./plugin-vue_export-helper-x3n3nnut.js";import{r,o as s,c as l,a as t,b as e,d as o,e as a}from"./app-MsA2k2kn.js";const c={},d=a('<h1 id="opencv实战-7-使用yolov3和opencv进行基于深度学习的目标检测" tabindex="-1"><a class="header-anchor" href="#opencv实战-7-使用yolov3和opencv进行基于深度学习的目标检测" aria-hidden="true">#</a> [OpenCV实战]7 使用YOLOv3和OpenCV进行基于深度学习的目标检测</h1><p>在这篇文章中，我们将学习如何在OpenCV上使用YOLOv3（目标检测网络）。YOLOv3是检测算法YOLO的最新变种已发布的模型可识别图像和视频中的80个不同对象，但最重要的是它具有超快速且几乎与Single Shot MultiBox（SSD）一样准确。从OpenCV 3.4.2开始，您可以在自己的OpenCV应用程序中轻松使用YOLOv3模型。</p><p>我们可以将对象检测器视为对象定位器和对象识别器的组合。</p><h2 id="_1-yolo-介绍" tabindex="-1"><a class="header-anchor" href="#_1-yolo-介绍" aria-hidden="true">#</a> <strong>1 YOLO</strong> <strong>介绍</strong></h2><h3 id="_1-1-yolov3-原理" tabindex="-1"><a class="header-anchor" href="#_1-1-yolov3-原理" aria-hidden="true">#</a> <strong>1.1 YOLOv3</strong> <strong>原理</strong></h3><p>在传统的计算机视觉方法中，使用滑动窗口来寻找不同位置和尺度的物体。因为这是非常耗时的操作，所以通常假设物体的纵横比是固定的。基于早期深度学习的对象检测算法（如R- CNN和Fast R-CNN）使用称为选择性搜索的方法来扫描图像。另一种称为Overfeat的方法涉及使用卷积式滑动窗口机制在多个尺度上扫描图像。紧随其后的是更快的R- CNN，它使用区域提议网络（RPN）来识别需要测试的边界框。通过巧妙的设计，提取用于识别对象的特征也被RPN用于提出潜在的边界框，从而节省了大量的计算。</p><p>Faster R-CNN使用区域候选网络（RPN）来识别需要测试的边界框。通过巧妙的设计，提取用于识别对象的特征也被RPN用于提出潜在的边界框，从而节省了大量的计算。以上的目标检测网络都是两阶段检测器，目标定位和分类是分开的。</p><p>接下来说说单阶段检测器，也就是ssd和mobilenet。另一方面，YOLO以完全不同的方式处理对象检测问题。SSD是另一种物体检测算法。YOLOv3和ssd只要获取图像就能直接得到目标检测结果，但YOLOv3比SSD快得多，同时实现了非常不错的精度。YOLOv3在M40，TitanX或1080 Ti GPU上速度很快。</p><p>YOLO目标检测器首先，它将图像划分为13×13的单元格。这169个单元的大小取决于输入的大小。对于我们在实验中使用的416×416输入尺寸，单元尺寸为32×32。然后每个单元格作为一个边界框进行一次检测。对于每个边界框，网络还预测边界框实际包围对象的置信度，以分类的概率。大多数这些边界框都被消除了，因为它们的置信度很低，或者因为它们与另一个具有非常高置信度得分的边界框包围相同的对象。该技术称为非极大值抑制。</p><p>YOLOv3作者使YOLOv3比以前的作品YOLOv2更快，更准确。YOLOv3可以更好地进行多个尺度检测。他们还通过增加网络来改进网络。</p><h3 id="_1-2-为什么要将opencv-用于yolo" tabindex="-1"><a class="header-anchor" href="#_1-2-为什么要将opencv-用于yolo" aria-hidden="true">#</a> <strong>1.2</strong> <strong>为什么要将OpenCV</strong> <strong>用于YOLO</strong> <strong>？</strong></h3><p>以下是您可能希望将OpenCV用于YOLO的几个原因:</p><ol><li>与OpenCV应用程序轻松集成：如果您的应用程序已经使用OpenCV并且您只想使用YOLOv3，则无需担心编译和构建额外的Darknet代码。</li><li>OpenCV CPU版本快9倍：OpenCV的DNN模块CPU实现速度惊人。例如，与OpenMP一起使用时，Darknet在CPU上花费大约2秒钟来对单个图像进行推理。相比之下，OpenCV的实现只需0.22秒！查看下表。</li><li>Python支持：Darknet是用C语言编写的，并没有正式支持Python。相比之下，OpenCV确实如此。虽然有Darknet可用的python端口。</li></ol><h3 id="_1-3-在darknet-和opencv-上对yolov3-进行速度测试" tabindex="-1"><a class="header-anchor" href="#_1-3-在darknet-和opencv-上对yolov3-进行速度测试" aria-hidden="true">#</a> <strong>1.3</strong> <strong>在Darknet</strong> <strong>和OpenCV</strong> <strong>上对YOLOv3</strong> <strong>进行速度测试</strong></h3><p>下表显示了YOLOv3在Darknet与OpenCV上的性能。所有情况下的输入大小为416×416。毫无疑问，Darknet的GPU版本优于其他任何东西。使用OpenMP（并行计算工具）的Darknet比没有OpenMP的Darknet工作得更好也不足为奇，因为OpenMP允许使用多个处理器。令人惊讶的是，OpenCV的DNN CPU实现速度比使用OpenML的Darknet快9倍。</p><table><thead><tr><th>OS</th><th>Framework</th><th>CPU/GPU</th><th>Time(ms)/Frame</th></tr></thead><tbody><tr><td>Linux 16.04</td><td>Darknet</td><td>12x Intel Core i7-6850K CPU @ 3.60GHz</td><td>9370</td></tr><tr><td>Linux 16.04</td><td>Darknet + OpenMP</td><td>12x Intel Core i7-6850K CPU @ 3.60GHz</td><td>1942</td></tr><tr><td>Linux 16.04</td><td>OpenCV [CPU]</td><td>12x Intel Core i7-6850K CPU @ 3.60GHz</td><td>220</td></tr><tr><td>Linux 16.04</td><td>Darknet</td><td>NVIDIA GeForce 1080 Ti GPU</td><td>23</td></tr><tr><td>macOS</td><td>DarkNet</td><td>2.5 GHz Intel Core i7 CPU</td><td>7260</td></tr><tr><td>macOS</td><td>OpenCV [CPU]</td><td>2.5 GHz Intel Core i7 CPU</td><td>400</td></tr></tbody></table><p>OpenCV的DNN GPU仅使用英特尔的GPU进行测试，因此如果您没有英特尔GPU，代码会将您切换回CPU。所以A卡就算了。</p><h2 id="_2-使用yolov3-进行对象检测-c-python" tabindex="-1"><a class="header-anchor" href="#_2-使用yolov3-进行对象检测-c-python" aria-hidden="true">#</a> <strong>2</strong> <strong>使用YOLOv3</strong> <strong>进行对象检测（C++/Python</strong> <strong>）</strong></h2><h3 id="_2-1-模型及配置文件下载" tabindex="-1"><a class="header-anchor" href="#_2-1-模型及配置文件下载" aria-hidden="true">#</a> <strong>2.1</strong> <strong>模型及配置文件下载</strong></h3><p>首先进行检测需要下载yolov3.weights文件（包含预先训练的网络权重），yolov3.cfg文件（包含网络配置）和coco.names文件，其中包含COCO数据集中使用的80个不同的类名。下载地址分别如下：</p>',20),h={href:"https://pjreddie.com/media/files/yolov3.weights",target:"_blank",rel:"noopener noreferrer"},p={href:"https://github.com/pjreddie/darknet/blob/master/cfg/yolov3.cfg",target:"_blank",rel:"noopener noreferrer"},f={href:"https://github.com/pjreddie/darknet/blob/master/data/coco.names",target:"_blank",rel:"noopener noreferrer"},m=a(`<h3 id="_2-2-初始化参数" tabindex="-1"><a class="header-anchor" href="#_2-2-初始化参数" aria-hidden="true">#</a> <strong>2.2</strong> <strong>初始化参数</strong></h3><p>YOLOv3算法会生成检测框作为预测的输出。每个预测的框都有一个置信度。在第一阶段，忽略置信度较低的框以进行进一步处理。其余的框基于非极大性抑制，这消除了多余的重叠边界框。非最大抑制由参数nmsThreshold控制。您可以尝试更改这些值，并查看输出预测框的数量如何变化。</p><p>检测之前，我们会设置网络输入图像的输入宽度（inpWidth）和高度（inpHeight）的默认值。我们将宽高设置为416，以便我们可以将我们的运行与YOLOv3作者给出的Darknet的C代码进行比较。您也可以将它们更改为320以获得更快的结果，或者更改为608以获得更准确的结果。</p><h3 id="_2-3-加载模型和获取输入图像" tabindex="-1"><a class="header-anchor" href="#_2-3-加载模型和获取输入图像" aria-hidden="true">#</a> <strong>2.3</strong> <strong>加载模型和获取输入图像</strong></h3><p>文件coco.names包含训练模型的所有类别名。接下来，我们加载网络有两个部分：</p><p>yolov3.weights：预训练的重量。</p><p>yolov3.cfg：配置文件。</p><p>我们在这里将DNN后端设置为OpenCV，将目标设置为CPU。您可以尝试将首选目标设置为cv.dnn.DNN_TARGET_OPENCL以在GPU上运行它。但请记住，目前的OpenCV版本仅使用英特尔的GPU进行测试，如果您没有英特尔GPU，它会自动切换到CPU。</p><p>然后我们将读取图像，视频流或网络摄像头图像。此外，我们还保存检测结果。</p><p>C++代码如下：</p><pre><code>	// Give the configuration and weight files for the model 模型参数文件
...
</code></pre><h4 id="_2-4-3-画预测结果框格" tabindex="-1"><a class="header-anchor" href="#_2-4-3-画预测结果框格" aria-hidden="true">#</a> 2.4.3 画预测结果框格</h4><p>最后，我们在输入框架上绘制通过非最大性抑制过滤的框，并指定类别标签和置信度分数。</p><p>C++代码如下：</p><pre><code>/**
...
`)],-1),O=t("p",null,"python代码：",-1),x=t("pre",null,[t("code",null,`# This code is written at BigVision LLC. It is based on the OpenCV project. It is subject to the license terms in the LICENSE file found in this distribution and at http://opencv.org/license.html
...
`)],-1),w=t("h2",{id:"_4-参考",tabindex:"-1"},[t("a",{class:"header-anchor",href:"#_4-参考","aria-hidden":"true"},"#"),e(" 4 参考")],-1),P={href:"https://www.learnopencv.com/deep-learning-based-object-detection-using-yolov3-with-opencv-python-c/",target:"_blank",rel:"noopener noreferrer"};function C(N,L){const n=r("ExternalLinkIcon");return s(),l("div",null,[d,t("p",null,[t("a",h,[e(" https://pjreddie.com/media/files/yolov3.weights"),o(n)])]),t("p",null,[t("a",p,[e(" https://github.com/pjreddie/darknet/blob/master/cfg/yolov3.cfg"),o(n)])]),t("p",null,[t("a",f,[e(" https://github.com/pjreddie/darknet/blob/master/data/coco.names"),o(n)])]),m,t("p",null,[t("a",u,[e(" https://github.com/luohenyueji/OpenCV-Practical-Exercise "),o(n)])]),t("p",null,[t("a",g,[e(" https://download.csdn.net/download/luohenyj/11017030 "),o(n)])]),b,_,v,y,O,x,w,t("ul",null,[t("li",null,[t("a",P,[e(" https://www.learnopencv.com/deep-learning-based-object-detection-using-yolov3-with-opencv-python-c/ "),o(n)])])])])}const I=i(c,[["render",C],["__file","2019-03-13-_OpenCV实战_7 使用YOLOv3和OpenCV进行基于深度学习的目标检测.html.vue"]]);export{I as default};
