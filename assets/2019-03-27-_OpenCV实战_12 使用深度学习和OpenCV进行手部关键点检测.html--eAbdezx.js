import{_ as i,r as a,o as r,c as p,a as t,b as e,d as o,e as l}from"./app-CD_-ggd_.js";const c={},s=t("h1",{id:"opencv实战-12-使用深度学习和opencv进行手部关键点检测",tabindex:"-1"},[t("a",{class:"header-anchor",href:"#opencv实战-12-使用深度学习和opencv进行手部关键点检测"},[t("span",null,"[OpenCV实战]12 使用深度学习和OpenCV进行手部关键点检测")])],-1),m=t("p",null,"手部关键点检测是在手指上找到关节以及在给定图像中找到指尖的过程。它类似于在脸部（面部关键点检测）或身体（人体姿势估计）上找到关键点。但是手部检测不同的地方在于，我们将整个手部视为一个对象。",-1),h=t("p",null,"美国卡耐基梅隆大学智能感知实验室(CMU Perceptual Computing Lab)发布了手的关键点检测模型。详情见：",-1),_={href:"https://arxiv.org/pdf/1704.07809.pdf",target:"_blank",rel:"noopener noreferrer"},d=t("p",null,"我们将在本文介绍如何调用该模型。",-1),u=t("h2",{id:"_1-背景",tabindex:"-1"},[t("a",{class:"header-anchor",href:"#_1-背景"},[t("span",null,[t("strong",null,"1"),e(),t("strong",null,"背景")])])],-1),f=t("figure",null,[t("img",{src:"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]12 使用深度学习和OpenCV进行手部关键点检测/20190327165421835.jpeg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0x1b2hlbllK,size_16,color_FFFFFF,t_70",alt:"",tabindex:"0",loading:"lazy"}),t("figcaption")],-1),g=t("p",null,"上图出自上面说的论文",-1),E={href:"https://arxiv.org/pdf/1602.00134.pdf",target:"_blank",rel:"noopener noreferrer"},b=t("p",null,"他们将这些图像传递通过检测器，以获得许多粗略的关键点预测。一旦从不同视图获得同一手的检测到的关键点，就会执行关键点三角测量以获得关键点的3D位置。关键点的3D位置用于通过从3D到2D的重投影来稳健地预测关键点。这对于难以预测关键点的图像尤其重要。通过这种方式，他们可以在几次迭代中获得更好的检测器。",-1),B=t("p",null,"总之，他们使用关键点检测器和多视图图像来提出改进的检测器。改进的主要来源是标记的图像集的多视图图像。",-1),F=t("p",null,"该模型产生22个关键点。手有21个关键点(0到20号关键点)，而第22个关键点代表背景。关键点位置如下图所示：",-1),A=t("figure",null,[t("img",{src:"https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]12 使用深度学习和OpenCV进行手部关键点检测/20190327165422310.jpeg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0x1b2hlbllK,size_16,color_FFFFFF,t_70",alt:"",tabindex:"0",loading:"lazy"}),t("figcaption")],-1),y=t("h2",{id:"_2-实现",tabindex:"-1"},[t("a",{class:"header-anchor",href:"#_2-实现"},[t("span",null,"2 实现")])],-1),C=t("p",null,"从此链接下载该模型：",-1),x={href:"http://posefs1.perception.cs.cmu.edu/OpenPose/models/hand/pose_iter_102000.caffemodel",target:"_blank",rel:"noopener noreferrer"},v=l(`<p>这是一个caffe模型。</p><p>模型读取预测代码和其他caffe模型一样，如下所示：</p><pre><code>	//模型文件位置
...
`)],-1),L=t("p",null,"python代码：",-1),P=t("pre",null,[t("code",null,`from __future__ import division
import cv2
import time
import numpy as np
