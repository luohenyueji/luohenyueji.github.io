...
</code></pre><p>从代码可以看出，程序先是读入一段视频数据流。通过fread函数读取w<em>h</em>3/2个unsigned char长度的数据实现第一帧图像的读取，unsigned<br> char占一个字节（通过sizeof(unsigned char）可以查看到)，也就是说fread函数读取w<em>h</em>3/2字节的数据就可以实现一帧图像的读取。</p><p>其中这段代码的fread函数是指每次读取1个字节的数据，一共读取w<em>h(y的长度)+(w/2</em>h/2)(u的长度)+<br> (w/2<em>h/2)(v的长度)=w</em>h*3/2次。</p><pre><code>fread(pic, sizeof(unsigned char), w * h * 3 / 2, input_fp);
