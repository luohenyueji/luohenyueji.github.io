...
</code></pre><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 序列化实例</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 保存到本地，这个文件名包含后缀可以随意命名，反正是二进制文件</span>
...
Vdata struct
p2
</code></pre><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token comment"># 保存到本地，这个文件名包含后缀可以随意命名，反正是二进制文件</span>
<span class="token keyword">with</span> <span class="token builtin">open</span><span class="token punctuation">(</span><span class="token string">&#39;data.bin&#39;</span><span class="token punctuation">,</span> <span class="token string">&#39;wb&#39;</span><span class="token punctuation">)</span> <span class="token keyword">as</span> f<span class="token punctuation">:</span>
    <span class="token comment"># 设置最底层协议</span>
...
</code></pre><p>如果想反序列化，重新读入文件，直接用pickle.load函数就行了。序列化协议是自动检测的，不需要指定。此外还有两个参数encoding和errors告诉pickle如何反序列低于当前python版本的序列化文件，默认值就行了。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> pickle
...
14
[1, 2, 3]
</code></pre><p>序列化和反序列化的过程可以通过__getstate__ 和__setstate__函数来影响。其中__getstate__函数在序列化时调用，__setstate__函数在反序列化时调用。</p><p>一个实例如下，在序列化时指定序列化某些参数，反序列化时恢复参数。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> pickle
...
</code></pre><p><strong>使用pickle最高协议</strong></p><p>将参数协议指定为-1，即可，但是加速可能效果不明显。具体看数据。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> time
<span class="token keyword">import</span> pickle
<span class="token keyword">import</span> numpy <span class="token keyword">as</span> np
<span class="token keyword">import</span> os
...
</code></pre><p><strong>用cPickle代替pickle</strong></p><p>最简单方式是使用cPickle而不是pickle。cPickle与pickle是完全相同的模块，具有相同的功能、相同的参数。唯一区别是cPickle用C语言编写的，这使cPickle速度更快。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> time
...
</code></pre><p><strong>禁用垃圾回收</strong></p><p>垃圾收集器会减慢处理速度，禁用它可以提高性能。</p><div class="language-python line-numbers-mode" data-ext="py" data-title="py"><pre class="language-python"><code><span class="token keyword">import</span> time
<span class="token keyword">import</span> pickle
<span class="token keyword">import</span> numpy <span class="token keyword">as</span> np
<span class="token keyword">import</span> os
...
</code></pre><h2 id="_3-参考" tabindex="-1"><a class="header-anchor" href="#_3-参考"><span>3 参考</span></a></h2>`,44),r={href:"https://www.tutorialandexample.com/python-pickle",target:"_blank",rel:"noopener noreferrer"},v={href:"https://zetcode.com/python/pickle/",target:"_blank",rel:"noopener noreferrer"},m={href:"https://vmartin.fr/how-to-make-pickle-faster.html",target:"_blank",rel:"noopener noreferrer"},b={href:"https://python.readthedocs.io/en/stable/library/pickle.html",target:"_blank",rel:"noopener noreferrer"};function y(h,_){const a=o("ExternalLinkIcon");return c(),i("div",null,[u,n("p",null,[s("对于pickle的协议选取，目前有5种不同的协议可用(出自"),n("a",k,[s("Python object serialization"),t(a)]),s(")。使用的协议越高，读取生成的pickle所需的Python版本越新。这些协议包括：")]),d,n("ul",null,[n("li",null,[n("a",r,[s("Python Pickle"),t(a)])]),n("li",null,[n("a",v,[s("Python pickle"),t(a)])]),n("li",null,[n("a",m,[s("How to make pickle faster?"),t(a)])]),n("li",null,[n("a",b,[s("Python object serialization"),t(a)])])])])}const g=e(l,[["render",y],["__file","2022-07-07-_python_ ​Python数据序列化模块pickle使用笔记.html.vue"]]),w=JSON.parse('{"path":"/blog/python/python%E5%AD%A6%E4%B9%A0/2022-07-07-_python_%20%E2%80%8BPython%E6%95%B0%E6%8D%AE%E5%BA%8F%E5%88%97%E5%8C%96%E6%A8%A1%E5%9D%97pickle%E4%BD%BF%E7%94%A8%E7%AC%94%E8%AE%B0.html","title":"[python] ​Python数据序列化模块pickle使用笔记","lang":"zh-CN","frontmatter":{"date":"2022-07-07T23:37:35.000Z","category":["Python"],"tag":["Python","编程基础"],"description":"[python] ​Python数据序列化模块pickle使用笔记 pickle是一个Python的内置模块，用于在Python中实现对象结构序列化和反序列化。Python序列化是一个将Python对象层次结构转换为可以本地存储或者网络传输的字节流的过程，反序列化则是将字节流还原为将Python对象层次结构。 数据序列化的功能简单理解为把不能直接存储的...","head":[["meta",{"property":"og:url","content":"https://luohenyueji.github.io/blog/python/python%E5%AD%A6%E4%B9%A0/2022-07-07-_python_%20%E2%80%8BPython%E6%95%B0%E6%8D%AE%E5%BA%8F%E5%88%97%E5%8C%96%E6%A8%A1%E5%9D%97pickle%E4%BD%BF%E7%94%A8%E7%AC%94%E8%AE%B0.html"}],["meta",{"property":"og:site_name","content":"落痕月极的博客"}],["meta",{"property":"og:title","content":"[python] ​Python数据序列化模块pickle使用笔记"}],["meta",{"property":"og:description","content":"[python] ​Python数据序列化模块pickle使用笔记 pickle是一个Python的内置模块，用于在Python中实现对象结构序列化和反序列化。Python序列化是一个将Python对象层次结构转换为可以本地存储或者网络传输的字节流的过程，反序列化则是将字节流还原为将Python对象层次结构。 数据序列化的功能简单理解为把不能直接存储的..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"article:author","content":"落痕月极"}],["meta",{"property":"article:tag","content":"Python"}],["meta",{"property":"article:tag","content":"编程基础"}],["meta",{"property":"article:published_time","content":"2022-07-07T23:37:35.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"[python] ​Python数据序列化模块pickle使用笔记\\",\\"image\\":[\\"\\"],\\"datePublished\\":\\"2022-07-07T23:37:35.000Z\\",\\"dateModified\\":null,\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"落痕月极\\",\\"url\\":\\"/\\"}]}"]]},"headers":[{"level":2,"title":"1 pickle使用","slug":"_1-pickle使用","link":"#_1-pickle使用","children":[]},{"level":2,"title":"2 pickle加速","slug":"_2-pickle加速","link":"#_2-pickle加速","children":[]},{"level":2,"title":"3 参考","slug":"_3-参考","link":"#_3-参考","children":[]}],"git":{},"readingTime":{"minutes":7.66,"words":2298},"filePathRelative":"blog/python/python学习/2022-07-07-[python] ​Python数据序列化模块pickle使用笔记.md","localizedDate":"2022年7月8日","excerpt":"\\n<p>pickle是一个Python的内置模块，用于在Python中实现对象结构序列化和反序列化。Python序列化是一个将Python对象层次结构转换为可以本地存储或者网络传输的字节流的过程，反序列化则是将字节流还原为将Python对象层次结构。</p>\\n<p>数据序列化的功能简单理解为把不能直接存储的数据存储到磁盘中，从而延长对象的生命周期。Python的常用序列化库有两个，即json和pickle。json库和pickle库的主要区别有两点：</p>\\n<ul>\\n<li>pickle可以序列化Python中所有的数据类型，包括类，函数，一般存储为二进制文件。而json只能序列化Python基本的数据类型，转储结果非常容易阅读。</li>\\n<li>pickle只能在Python中使用，而json是能够在不同语言之间交换数据的。</li>\\n</ul>","autoDesc":true}');export{g as comp,w as data};
