...
if __name__ == &#39;__main__&#39;:
    ...
    cv2.waitKey(0)
</code></pre><p>结果就是本文一张图的效果。</p><p>如果您要实际制作商业图像注册产品，则需要做的比我的代码更多。例如，当对齐错误很多时，此代码可能会失败。在这种情况下，您需要在较低分辨率版本的图像上估计变换参数，并使用低分辨率版本中估计的参数初始化较高分辨率版本的变换矩阵。此外，findTransformECC估计单个全局变换以进行对齐。当图像中存在局部运动时（例如，主体在两张照片中移动了一点），这种运动模型显然是不够的。在这种情况下，需要使用基于光流的方法来进行额外的局部对准。另外这个算法计算很慢，注意图像分辨率。如果想提高精度，改变阈值增加算法迭代次数。</p><h2 id="_3-参考" tabindex="-1"><a class="header-anchor" href="#_3-参考"><span>3 参考</span></a></h2><ul><li><a href="https://www.learnopencv.com/image-alignment-ecc-in-opencv-c-python/" target="_blank" rel="noopener noreferrer"> https://www.learnopencv.com/image-alignment-ecc-in-opencv-c-python/</a></li></ul>`,54)]])}var s=r(a,[[`render`,o]]);export{i as _pageData,s as default};