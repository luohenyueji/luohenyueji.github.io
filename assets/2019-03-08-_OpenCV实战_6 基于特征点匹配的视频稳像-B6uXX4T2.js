...
</code></pre><h4 id="_2-2-2-计算平滑轨迹" tabindex="-1"><a class="header-anchor" href="#_2-2-2-计算平滑轨迹"><span>2.2.2 计算平滑轨迹</span></a></h4><p>在上一步中，我们计算了运动的轨迹。因此，我们有三条曲线显示运动（x，y和角度）随时间的变化情况。</p><p>在这一步中，我们将展示如何平滑这三条曲线。</p><p>平滑任何曲线的最简单方法是使用移动平均滤波器。顾名思义，移动平均滤波器将该点处的函数值替换为邻域窗格所有点的平均值。我们来看一个例子。</p><p>比方说，我们已经存储在数组中的曲线C，曲线上的点为C [0], ... ,C [N-1]。，用窗宽为5的移动平均滤波器对曲线c滤波可以得到平滑的曲线 <em>f</em> 。计算公式如下：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]6 基于特征点匹配的视频稳像/4.png" alt="" tabindex="0" loading="lazy"></figure><p>如下图，平滑曲线的值是在小窗口上平均左侧噪声曲线的值。下图显示了左侧噪声曲线的示例，使用右侧大小为5移动平均滤波器进行平滑处理。</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]6 基于特征点匹配的视频稳像/5.png" alt="" tabindex="0" loading="lazy"></figure><p>在C ++版本中，我们定义了一个名为smooth的函数，它计算平滑的移动平均轨迹。</p><pre><code>/**
 * @brief 平滑运动轨迹
 *
...
</code></pre><h3 id="_2-3-将平滑后的变化矩阵应用于帧" tabindex="-1"><a class="header-anchor" href="#_2-3-将平滑后的变化矩阵应用于帧"><span><strong>2.3</strong> <strong>将平滑后的变化矩阵应用于帧</strong></span></a></h3><p>我们差不多完成了。我们现在需要做的就是遍历帧并应用我们刚刚计算的变换。</p><p>如果我们将运动指定为 <em>x</em> , <em>y</em> , <em>θ</em> <img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]6 基于特征点匹配的视频稳像/6.png" alt="" loading="lazy"> ，则相应的变换矩阵由下式给出：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]6 基于特征点匹配的视频稳像/7.png" alt="" tabindex="0" loading="lazy"></figure><p>当我们稳定视频时，我们可能会看到一些黑色边界。这是预期的，因为要稳定视频，视频帧原图像可能不得不缩小（不是图的尺寸缩小，图像尺寸不变。有两种情况，一种实际先缩小图像，然后从中截取原图大小的区域，缺少图像区域用黑色填充，起到图像增大作用；另外将原图扩大，然后截取原图尺寸相等大小区域，起到图像缩小作用）。我们可以通过以其视频中点为中心缩放图像（例如4％）来缓解该问题。下面的函数fixBorder显示了实现。我们使用getRotationMatrix2D，因为它可以在不移动图像中心的情况下缩放和旋转图像。我们需要做的就是调用此函数，旋转0和缩放1.04(将原图扩大为1.04倍，然后截取原图尺寸相等大小区域)。C++代码如下：</p><pre><code>/**
 * @brief 
 * 
