import{_ as i,r as l,o as t,c as d,a as n,b as s,d as a,e as v}from"./app-q9tmr8t5.js";const r={},c=v(`<h1 id="编程基础-c-多线程入门5-使用互斥锁解决资源竞争" tabindex="-1"><a class="header-anchor" href="#编程基础-c-多线程入门5-使用互斥锁解决资源竞争" aria-hidden="true">#</a> [编程基础] C++多线程入门5-使用互斥锁解决资源竞争</h1><p>原始C++标准仅支持单线程编程。新的C++标准（称为C++11或C++0x）于2011年发布。在C++11中，引入了新的线程库。因此运行本文程序需要C++至少符合C++11标准。</p><h2 id="_5-使用互斥锁解决资源竞争" tabindex="-1"><a class="header-anchor" href="#_5-使用互斥锁解决资源竞争" aria-hidden="true">#</a> 5 使用互斥锁解决资源竞争</h2><p>在本文中，我们将讨论如何使用互斥锁来保护多线程环境中的共享数据并避免资源竞争。为了解决多线程环境中的资源竞争，我们需要互斥锁，即每个线程都需要在修改或读取共享数据之前锁定互斥锁，并且在修改数据之后，每个线程都应解锁互斥锁。</p><h3 id="_5-1-std-mutex" tabindex="-1"><a class="header-anchor" href="#_5-1-std-mutex" aria-hidden="true">#</a> 5.1 std::mutex</h3><p>在C++11线程库中，互斥锁位于mutex头文件中。表示互斥锁的类是std::mutex类 互斥锁有两种重要的方法：</p><ol><li>lock()</li><li>unlock()</li></ol><p>我们已经在上一篇文章中使用多线程钱包解释了资源竞争。在本文中，我们将看到如何使用std::mutex修复该多线程钱包中的资源竞争。由于电子钱包提供了在电子钱包中添加资金的服务，并且在不同线程之间使用了相同的电子钱包对象，因此我们需要在电子钱包的addMoney()方法中添加锁定，即在增加电子钱包的货币之前获取锁并在离开该钱包之前释放锁功能。让我们看一下代码： 内部维护货币并提供服务/功能的钱包类，即addMoney()。 该成员函数首先获取一个锁，然后将钱包对象的内部货币增加指定的数量，然后释放该锁。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>#include&lt;iostream&gt;
#include&lt;thread&gt;
#include&lt;vector&gt;
#include&lt;mutex&gt;
