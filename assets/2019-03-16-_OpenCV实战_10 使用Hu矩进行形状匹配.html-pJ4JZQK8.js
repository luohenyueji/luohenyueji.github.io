import{_ as i,r as p,o as l,c as r,a as e,b as t,d as a,e as o}from"./app-CD_-ggd_.js";const u={},s=o('<h1 id="opencv实战-10-使用hu矩进行形状匹配" tabindex="-1"><a class="header-anchor" href="#opencv实战-10-使用hu矩进行形状匹配"><span>[OpenCV实战]10 使用Hu矩进行形状匹配</span></a></h1><p>在这篇文章中，我们将展示如何使用Hu Moments进行形状匹配。您将学习以下内容</p><ul><li>什么是图像矩？</li><li>如何计算图像矩？</li><li>什么是图像矩不变量（或Hu时刻）？</li><li>如何使用OpenCV计算图像的Hu图像矩？</li><li>如何使用Hu图像矩来找到两个形状之间的相似性。</li></ul><h2 id="_1-什么是图像矩" tabindex="-1"><a class="header-anchor" href="#_1-什么是图像矩"><span><strong>1</strong> <strong>什么是图像矩？</strong></span></a></h2><p>图像矩是图像像素强度的加权平均值。让我们选择一个简单的例子来理解。</p><p>为简单起见，我们考虑单通道二进制图像 <em>I</em> 。位置处的像素强度（X，Y）为 <em>I</em> （X，Y）。二进制图像的I（X，Y）可以取值0或255。</p><p>最简单的图像矩可以这样计算：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/20190316185220106.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>我们在上面的等式中所做的就是计算所有像素值的总和。换句话说，所有图像矩仅基于它们的值加权，而不是基于它们在图像中的位置。</p><p>对于二进制图像，可以以几种不同的方式解释上述矩:</p><ol><li>它是值白色像素的数量（即强度=255）。</li><li>它是代表图像中白色区域的面积。</li></ol><p>到目前为止，您可能不会对图像矩留下深刻印象，但这里有一些有趣的东西。图1包含三个二进制图像S（S0.png），旋转S（S5.png）和K（K0.png）</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/20190316185220114.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L0x1b2hlbllK,size_16,color_FFFFFF,t_70" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>S图像和旋转S图像的图像矩非常接近，K的矩就大大不同。</p><p>对于两个相同的形状，上面的图像矩必然是相同的，但它不是一个充分的条件。我们可以很容易地构建两个图像，其中图像矩相同的，但它们看起来非常不同。</p><h2 id="_2-如何计算图像矩" tabindex="-1"><a class="header-anchor" href="#_2-如何计算图像矩"><span><strong>2</strong> <strong>如何计算图像矩</strong></span></a></h2><h3 id="_2-1-质心获取" tabindex="-1"><a class="header-anchor" href="#_2-1-质心获取"><span><strong>2.1</strong> <strong>质心获取</strong></span></a></h3><p>让我们看看一些更复杂的矩。</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/20190316185220118.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>i和j是整数。这种矩通常被称为图像几何矩，以区别于本文后面提到的中心矩。请注意，上述矩取决于像素的强度及其在图像中的位置。如此直观地说，这些矩正在捕捉一些形状的信息。</p><p>我们可以通过图像矩计算质心。使用以下公式计算质心：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/20190316185220121.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/20190316185220129.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>其他信息查看：</p>',24),c={href:"https://blog.csdn.net/LuohenYJ/article/details/88599334",target:"_blank",rel:"noopener noreferrer"},m=o('<h3 id="_2-2-中心矩" tabindex="-1"><a class="header-anchor" href="#_2-2-中心矩"><span><strong>2.2</strong> <strong>中心矩</strong></span></a></h3><p>中心矩非常类似于我们之前看到的几何矩，在几何矩的基础上我们需要减去质心坐标。</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/20190316185220139.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>请注意，上述中心矩是具有平移不变性的。换句话说，无论图像中的blob在哪里，如果形状相同，则中心矩是不变的。</p><p>如果我们还能让这个矩具有不变性，那会不会很酷？那么为此我们需要在中心矩添加标准化，得到中心归一化矩。如下所示。</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/20190316185220140.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>中心矩是平移不变的，中心归一化矩是平移和尺度不变的。三种矩总结如下：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/934484-20170713101542165-2053888115.png" alt="https://images2015.cnblogs.com/blog/934484/201707/934484-20170713101542165-2053888115.png" tabindex="0" loading="lazy"><figcaption>https://images2015.cnblogs.com/blog/934484/201707/934484-20170713101542165-2053888115.png</figcaption></figure><h3 id="_2-3-hu矩" tabindex="-1"><a class="header-anchor" href="#_2-3-hu矩"><span>2.3 Hu矩</span></a></h3><p>中心矩具有很不错的特性，但是不足以用于特征匹配。我们想要计算对平移，缩放和旋转不变的矩，幸运的是，我们实际上可以计算出这样的矩，他们被称为Hu矩的7个不变量。如下图所示。</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]10 使用Hu矩进行形状匹配/20190316185220150.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>Hu矩（或者更确切地说是Hu矩不变量）是使用对图像变换不变的中心矩计算的一组7个变量。事实证明，前6个矩不变量对于平移，缩放，旋转和映射都是不变的。而第7个矩会因为图像映射而改变。</p><p>Hu矩的理论你可参考论文：</p>',13),h={href:"https://www.researchgate.net/publication/224146066_Analysis_of_Hu's_moment_invariants_on_image_scaling_and_rotation",target:"_blank",rel:"noopener noreferrer"},g=o(`<h2 id="_3-基于hu-矩实现形状匹配" tabindex="-1"><a class="header-anchor" href="#_3-基于hu-矩实现形状匹配"><span><strong>3</strong> <strong>基于Hu</strong> <strong>矩实现形状匹配</strong></span></a></h2><h3 id="_3-1-hu-矩的计算" tabindex="-1"><a class="header-anchor" href="#_3-1-hu-矩的计算"><span><strong>3.1 Hu</strong> <strong>矩的计算</strong></span></a></h3><p>幸运的是，我们不需要在OpenCV中进行所有计算，因为我们有计算Hu矩的函数。在OpenCV中，我们HuMoments()用来计算输入图像中的Hu矩。</p><p>（1）我们先读取原图并将其转换为灰度图像</p><p>C++:</p><pre><code>// Read image as grayscale image
...
</code></pre><p>如果您想在两个形状之间自定义距离。例如，您可能希望使用由给定的Hu Moments之间的欧几里德距离。首先，如前一节所述，计算对数变换的Hu矩，然后自己计算距离，而不是使用matchShapes。</p><h2 id="_4-代码" tabindex="-1"><a class="header-anchor" href="#_4-代码"><span>4 代码</span></a></h2><p>代码地址：</p>`,55),E={href:"https://github.com/luohenyueji/OpenCV-Practical-Exercise",target:"_blank",rel:"noopener noreferrer"},_={href:"https://download.csdn.net/download/luohenyj/11026231",target:"_blank",rel:"noopener noreferrer"},d=o(`<p>如果没有积分（系统自动设定资源分数）看看参考链接。我搬运过来的，大修改没有。</p><h3 id="_4-1-hu矩计算" tabindex="-1"><a class="header-anchor" href="#_4-1-hu矩计算"><span>4.1 Hu矩计算</span></a></h3><p>C++:</p><pre><code>#include &quot;pch.h&quot;
#include &lt;iostream&gt;
