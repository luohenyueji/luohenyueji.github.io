...
</code></pre><p>上面代码的结果如下：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]28 基于OpenCV的GUI库cvui/20190508174312652.gif" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>确保cvui与您的项目正常工作：</p><p>cvui::init()在创建任何组件之前调用初始化函数。</p><p>cvui::update() 在创建所有组件后调用一次。</p>`,23),a(`p`,null,[c[0]||=o(`关于上面代码中使用的组件， 每次单击按钮时cvui`,-1),e(p,{icon:`button()
函数都会返回true，因此您可以方便地在if语句中使用它。该cvui`}),c[1]||=o(`printf()功能与标准C功能的工作方式类似printf()，%d and`,-1),c[2]||=a(`br`,null,null,-1),c[3]||=o(` %s.分别表示文字和数字。您还可以使用十六进制值选择文本的颜色0xRRGGBB，例如0xFF0000（红色），0x00FF00（绿色）和0x0000FF（蓝色）。具体看代码就知道了。`,-1)]),c[5]||=i(`<h2 id="_2-更高级的应用" tabindex="-1"><a class="header-anchor" href="#_2-更高级的应用"><span><strong>2</strong> <strong>更高级的应用</strong></span></a></h2><p>现在让我们构建一些更复杂的东西，但就像以前一样容易。该应用程序将Canny<br> Edge算法应用于图像，允许用户启用/禁用该技术并调整其阈值。Canny边缘算法详细见：</p><p><a href="https://en.wikipedia.org/wiki/Canny_edge_detector" target="_blank" rel="noopener noreferrer"> https://en.wikipedia.org/wiki/Canny_edge_detector<br></a></p><p>(1) 基础</p><p>我们首先创建一个没有UI元素的应用程序。Canny<br> Edge算法的使用由布尔变量（use_canny）定义，而算法阈值由两个整数（low_threshold和high_threshold）定义。使用这种方法，我们必须在每次要启用/禁用canny或调整low_threshold和high_threshold时重新编译代码。</p><p>该部分代码如下：</p><pre><code>#include &lt;opencv2/opencv.hpp&gt;
#define CVUI_IMPLEMENTATION
#include &quot;cvui.h&quot;
...
</code></pre><p>结果在应用程序显示原始图像（use_canny设置为false）或显示检测到的边缘图像（use_canny设置true）。结果如下：</p><figure><img src="https://www.learnopencv.com/wp-content/uploads/2017/06/cvui-canny-manual-small.png" alt="https://www.learnopencv.com/wp-content/uploads/2017/06/cvui-canny-manual-small.png" tabindex="0" loading="lazy"><figcaption><a href="https://www.learnopencv.com/wp-content/uploads/2017/06/cvui-canny-manual-small.png" target="_blank" rel="noopener noreferrer">https://www.learnopencv.com/wp-content/uploads/2017/06/cvui-canny-manual-small.png</a></figcaption></figure><p>(2)动态启用/禁用边缘检测</p><p>让我们通过使用cvui并添加一个复选框来控制值use_canny。使用该方法，用户可以在应用程序仍在运行时启用/禁用Canny<br> Edge。我们添加所需的cvui代码并使用该cvui::checkbox函数：</p><p>仅仅这个小修改就可以节省测试应用程序的时间，而无需重新编译所有内容。代码如下：</p><pre><code>#include &lt;opencv2/opencv.hpp&gt;
#define CVUI_IMPLEMENTATION
#include &quot;cvui.h&quot;
...
</code></pre><p>结果如下：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]28 基于OpenCV的GUI库cvui/20190508174530181.gif" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>复选框及其标签显示效果取决于所使用的图像，某些背景下复选框可能无法显示。我们可以通过创建一个cvui::window()<br> 用来容纳复选框的窗口来防止这个问题。代码如下：</p><pre><code>#include &lt;opencv2/opencv.hpp&gt;
#define CVUI_IMPLEMENTATION
#include &quot;cvui.h&quot;
...
</code></pre><p>结果如下：</p><figure><img src="https://www.learnopencv.com/wp-content/uploads/2017/06/cvui-canny-decent-ui.png" alt="https://www.learnopencv.com/wp-content/uploads/2017/06/cvui-canny-decent-ui.png" tabindex="0" loading="lazy"><figcaption><a href="https://www.learnopencv.com/wp-content/uploads/2017/06/cvui-canny-decent-ui.png" target="_blank" rel="noopener noreferrer">https://www.learnopencv.com/wp-content/uploads/2017/06/cvui-canny-decent-ui.png</a></figcaption></figure><p>(3) 调整阈值</p><p>是时候允许用户在运行时选择low_threashold和high_threashold的值。由于这些参数可以在一个间隔内变化，我们可以cvui::trackbar()用来创建一个滑动窗口栏。代码如下：</p><pre><code>#include &lt;opencv2/opencv.hpp&gt;
#define CVUI_IMPLEMENTATION
#include &quot;cvui.h&quot;
...
</code></pre><p>该 cvui::trackbar() 函数接受指定轨迹栏允许的最小值和最大值的参数。在上面的例子中，它们分别是[5,150]<br> low_threshold和[80,300] high_threshold。结果是一个完全交互式的应用程序，允许用户快速，轻松地探索Canny Edge参数的调整，以及启用/禁用它的使用。效果如图所示：</p><figure><img src="https://gitlab.com/luohenyueji/article_picture_warehouse/-/raw/main/blog/[OpenCV实战]28 基于OpenCV的GUI库cvui/20190508174721447.gif" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><h2 id="_3-代码" tabindex="-1"><a class="header-anchor" href="#_3-代码"><span>3 代码</span></a></h2><p>以下是此应用程序的完整代码，您不需要多行代码就可以为您的应用程序生成最小（且有用）的UI界面。该cvui并非旨在成为复杂图形应用程序开发的完整解决方案。它在很多方面都很简单并且有限。但是，它实用，易于使用，可以为您节省数小时的挫折和繁琐的工作。如果实际工程应用推荐QT而不是MFC(个人观点)。</p><p>本文所有代码和cvui头文件见：</p><p><a href="https://github.com/luohenyueji/OpenCV-Practical-Exercise" target="_blank" rel="noopener noreferrer"> https://github.com/luohenyueji/OpenCV-Practical-Exercise<br></a></p><p>hello world代码：</p><pre><code>#include &quot;pch.h&quot;
